...
module.exports.observerControl = function (room) {
  ...
  if (observer) {
    if (observedRooms[room.name] && Game.rooms[observedRooms[room.name]]) {
      ...
      if (
        Game.map.getRoomLinearDistance(observedRooms[room.name], room.name) <= 2
      )
        Game.rooms[observedRooms[room.name]].invaderCheck();
    ...
    let scoutOperation = _.findKey(
      Memory.targetRooms,
      (t) => t && t.type === "attack"
    );
    if (
      scoutOperation &&
      Game.map.getRoomLinearDistance(room.name, scoutOperation) <=
        OBSERVER_RANGE
    ) {
    ...
    let observerOperation = _.findKey(
      Memory.targetRooms,
      (t) =>
        t &&
        t.level === 0 &&
        (!t.observerCheck || t.observerCheck + 50 < Game.time)
    );
    if (
      observerOperation &&
      Game.map.getRoomLinearDistance(room.name, observerOperation) <=
        OBSERVER_RANGE
    ) {
    ...
    let claimScoutOperation = _.findKey(
      Memory.targetRooms,
      (t) => t && t.type === "claimScout"
    );
    if (
      claimScoutOperation &&
      Game.map.getRoomLinearDistance(room.name, claimScoutOperation) <=
        OBSERVER_RANGE
    ) {
    ...
    if (Math.random() > 0.5) {
      ...
      target =
        room.name.replace(/[0-9]/g, "")[0] +
        targetX +
        room.name.replace(/[0-9]/g, "")[1] +
        targetY;
