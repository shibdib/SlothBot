...
Object.defineProperty(Room.prototype, 'energyState', {
    get: function () {
        ...
        let target = this.level === 8 ? 250000 : Math.max(this.level * 20000, Math.min(Math.round(upgradeCost * progressFraction), STORAGE_CAPACITY * 0.5));
...
Room.prototype.cacheRoomIntel = function (force = false, creep = undefined) {
    ...
    if (!roomIntel.microUpdate || roomIntel.microUpdate + 150 < currentTime) {
        ...
        // Armed hostiles
        roomIntel.armedHostile = this.hostileCreeps.some(c => c.hasActiveBodyparts(ATTACK) || c.hasActiveBodyparts(RANGED_ATTACK)) ? Game.time : undefined;
