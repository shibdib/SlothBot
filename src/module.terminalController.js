...
if (Memory._banker) spendingMoney = Memory._banker.spendingAccount;
else spendingMoney = 0;
...
module.exports.terminalControl = function (room) {
  ...
  if (runOnce !== Game.time) {
    if (Memory._banker) {
      if (spendingMoney > Game.market.credits - CREDIT_BUFFER)
        spendingMoney = Game.market.credits - CREDIT_BUFFER * 1.1;
      Memory._banker.spendingAccount = _.floor(spendingMoney, 1);
    }
    // Track profits
    profitCheck();
    // Reaction amount is 500 if we are low on cash
    if (Game.market.credits < CREDIT_BUFFER) reactionAmount = 500;
    else reactionAmount = REACTION_AMOUNT;
    // Get global orders
    ...
    if (
      ...
      Memory.saleTerminal.saleSet + 15000 < Game.time
    ) {
      // Clear if no longer valid
      if (
        !Game.rooms[Memory.saleTerminal.room] ||
        !Memory.roomCache[Memory.saleTerminal.room] ||
        Memory.roomCache[Memory.saleTerminal.room].owner !== MY_USERNAME
      )
        Memory.saleTerminal = {};
      if (
        Memory.saleTerminal.room &&
        Game.rooms[Memory.saleTerminal.room].controller.level ===
          Memory.maxLevel
      ) {
        return (Memory.saleTerminal.saleSet = Game.time);
      }
      Memory.saleTerminal.room = _.sample(
        _.filter(
          Game.structures,
          (s) =>
            s.structureType === STRUCTURE_TERMINAL &&
            !s.room.memory.praiseRoom &&
            s.room.level === Memory.maxLevel &&
            !s.room.nukes.length &&
            s.isActive() &&
  ...
  if (
    Memory.saleTerminal.room &&
    room.name === Memory.saleTerminal.room &&
    room.nukes.length
  ) {
    Memory.saleTerminal.room = undefined;
    return log.a(
      roomLink(room.name) +
        " is no longer the primary market room due to an incoming nuke."
    );
  }
  ...
  for (let key in myOrders) {
    ...
    if (!order.active) {
      if (Game.market.cancelOrder(order.id) === OK) {
        log.e(
          "Order Cancelled: " + order.id + " no longer active.",
          "MARKET: "
        );
        return true;
      }
    ...
    if (order.type === ORDER_BUY) {
      if (Game.market.credits < 50) {
        if (Game.market.cancelOrder(order.id) === OK) {
          log.e(
            "Order Cancelled: " + order.id + " due to low credits",
            "MARKET: "
          );
          return true;
        }
      ...
      let duplicate = _.filter(
        myOrders,
        (o) =>
          o.roomName === order.roomName &&
          o.resourceType === order.resourceType &&
          o.type === ORDER_BUY &&
          o.id !== order.id
      );
      if (duplicate.length) {
        duplicate.forEach((duplicateOrder) =>
          Game.market.cancelOrder(duplicateOrder.id)
        );
      }
      if (order.resourceType !== RESOURCE_ENERGY) {
        if (order.remainingAmount > tradeAmount) {
          if (Game.market.cancelOrder(order.id) === OK) {
            log.e(
              "Order Cancelled: " +
                order.id +
                " for exceeding the set trade amount (order amount/set limit) " +
        ...
        if (
          _.filter(
            myRooms,
            (r) => r.terminal && r.energy >= ENERGY_AMOUNT * 2
          )[0]
        ) {
          if (Game.market.cancelOrder(order.id) === OK) {
            log.e(
              "Order Cancelled: " +
                order.id +
                " we have a room with an energy surplus and do not need to purchase energy",
              "MARKET: "
            );
            return true;
          }
      ...
      if (order.amount === 0) {
        if (Game.market.cancelOrder(order.id) === OK) {
          log.e(
            "Order Cancelled: " + order.id + " - Order Fulfilled.",
            "MARKET: "
          );
          return true;
        }
      ...
      if (order.roomName !== Memory.saleTerminal.room) {
        if (Game.market.cancelOrder(order.id) === OK) {
          log.e(
            "Order Cancelled: " +
              order.id +
              " - Not the designated sale terminal.",
            "MARKET: "
          );
          return true;
        }
        ...
        if (!order.amount) {
          if (Game.market.cancelOrder(order.id) === OK) {
            log.e(
              "Order Cancelled: " +
                order.id +
                " - Not enough resources remaining in terminal.",
              "MARKET: "
            );
            return true;
          }
        ...
        if (Game.market.cancelOrder(order.id) === OK) {
          log.e(
            "Order Cancelled: " +
              order.id +
              " - Cancel sale of energy as we have a shortage in the room.",
            "MARKET: "
          );
          return true;
        }
    ...
    if (!Game.rooms[order.roomName]) {
      if (Game.market.cancelOrder(order.id) === OK) {
        log.e(
          "Order Cancelled: " + order.id + " we no longer own this room",
          "MARKET: "
        );
        return true;
      }
...
function pricingUpdateSell(globalOrders, myOrders) {
  for (let key in myOrders) {
    ...
    if (order.type === ORDER_SELL) {
      ...
      let competitorOrder = _.min(
        globalOrders.filter(
          (o) =>
            !_.includes(Memory.myRooms, o.roomName) &&
            o.resourceType === order.resourceType &&
            o.type === ORDER_SELL
        ),
        "price"
...
function manageSellOrders(myOrders) {
  for (let key in myOrders) {
    ...
    if (order.resourceType !== RESOURCE_ENERGY) {
      if (
        Game.rooms[order.roomName].terminal.store[order.resourceType] -
          order.remainingAmount >
        1500
      ) {
        let amount =
          ...
          if (Game.market.extendOrder(order.id, amount) === OK) {
            log.w(
              "Extended sell order " +
                ...
                order.resourceType +
                " in " +
                roomLink(order.roomName),
              "Market: "
            );
            spendingMoney -= order.price * amount * 0.05;
            log.w(
              "Remaining spending account amount - " + spendingMoney,
              "Market: "
            );
            return true;
          }
...
function placeSellOrders(terminal, globalOrders, myOrders) {
  for (let resourceType of Object.keys(terminal.store)) {
    ...
    if (resourceType === RESOURCE_BATTERY)
      sellAmount = terminal.room.store(resourceType) - 1000;
    if (_.includes(_.union(COMPRESSED_COMMODITIES), resourceType))
      sellAmount = terminal.room.store(resourceType) - 5000;
    if (_.includes(_.union(REGIONAL_0_COMMODITIES), resourceType))
      sellAmount = terminal.room.store(resourceType) - REACTION_AMOUNT * 0.5;
    if (
      _.includes(
        _.union(
    ...
    if (
      _.includes(
        _.union(TIER_1_BOOSTS, TIER_2_BOOSTS, TIER_3_BOOSTS, [RESOURCE_POWER]),
        resourceType
      )
    )
    ...
    if (resourceType === RESOURCE_POWER)
      sellAmount = terminal.room.store(resourceType) - 10000;
    // Sell
    let price = 8;
    let competitorOrder = _.min(
      globalOrders.filter(
        (order) =>
          !_.includes(Memory.myRooms, order.roomName) &&
    ...
    if (sellAmount > terminal.store[resourceType])
      sellAmount = terminal.store[resourceType];
    let cost = price * sellAmount * 0.05;
    if (cost > availableCash)
      sellAmount = _.round(availableCash / (price * 0.05));
    if (sellAmount > 0) {
      if (
...
function baseMineralOnDemandBuys(terminal, globalOrders) {
  for (let mineral of shuffle(BASE_MINERALS)) {
    ...
    if (stored < reactionAmount * 0.8) {
      ...
      if (Game.market.credits < CREDIT_BUFFER)
        _.floor((buyAmount *= Game.market.credits / CREDIT_BUFFER));
      let sellOrder = _.min(
        globalOrders.filter(
          (order) =>
      ...
      if (sellOrder.price * buyAmount > spendingMoney)
        buyAmount = _.floor(spendingMoney / sellOrder.price);
      if (sellOrder.id && buyAmount >= 50) {
        if (buyAmount > sellOrder.amount) buyAmount = sellOrder.amount;
        if (
          Game.market.deal(sellOrder.id, buyAmount, terminal.pos.roomName) ===
          OK
        ) {
          log.w(
            "Bought " +
...
function buyPower(terminal, globalOrders) {
  let stored =
    terminal.store[RESOURCE_POWER] +
      terminal.room.storage.store[RESOURCE_POWER] || 0;
  ...
  if (buyAmount >= 1000) {
    let sellOrder = _.min(
      globalOrders.filter(
        (order) =>
          ...
          order.type === ORDER_SELL &&
          order.remainingAmount >= buyAmount &&
    ...
    if (buyAmount >= 500 && sellOrder.id) {
      if (
        Game.market.deal(sellOrder.id, buyAmount, terminal.pos.roomName) === OK
      ) {
        log.w(
          "Bought " +
            buyAmount +
...
function buyEnergy(terminal, globalOrders) {
  if (terminal.room.energy < ENERGY_AMOUNT && spendingMoney) {
    ...
    if (sellOrder.price) {
      ...
      if (buyAmount > sellOrder.amount) buyAmount = sellOrder.amount;
      if (buyAmount * sellOrder.price > spendingMoney)
        buyAmount = _.round(spendingMoney / sellOrder.price);
      if (
        buyAmount >= 500 &&
        Game.market.deal(sellOrder.id, buyAmount, terminal.pos.roomName) === OK
      ) {
...
function fillBuyOrders(terminal, globalOrders) {
  ...
  for (let resourceType of sortedKeys) {
    ...
    if (sellAmount > 0) {
      ...
      if (buyer.id) {
        if (buyer.remainingAmount < sellAmount)
          sellAmount = buyer.remainingAmount;
        if (
          Game.market.calcTransactionCost(
            sellAmount,
            terminal.room.name,
        ...
        if (sellAmount * buyer.price >= 5) {
          switch (
            Game.market.deal(buyer.id, sellAmount, terminal.pos.roomName)
          ) {
            case OK:
              log.w(
                terminal.pos.roomName +
              ...
              log.w(
                "New spending account amount - " + spendingMoney,
                "Market: "
              );
              return true;
          }
      ...
      // Offload if we're overflowing
        ...
        ((!_.includes(ALL_COMMODITIES, resourceType) &&
          sellAmount >= DUMP_AMOUNT * 3) ||
          (_.includes(ALL_COMMODITIES, resourceType) &&
            sellAmount >= DUMP_AMOUNT * 4)) &&
        ...
        let randomRoom =
          ...
          _.sample(
            _.filter(
              Memory.roomCache,
              (r) => r.user && r.user !== MY_USERNAME && r.level >= 6
            )
          );
        if (randomRoom) {
              ...
              case OK:
                log.w(
                  terminal.pos.roomName +
                    ...
                    roomLink(randomRoom) +
                    " (OWNED BY- " +
                    Memory.roomCache[randomRoom].user +
                    ") from " +
                    roomLink(terminal.room.name),
                  "Market: "
                );
...
function balanceResources(terminal) {
  ...
  for (let resource of sortedKeys) {
    ...
    // Send all of these
    if (
      _.includes(ALL_COMMODITIES, resource) ||
    ...
    if (_.includes(BASE_MINERALS, resource)) {
      let lab = _.filter(
        terminal.room.structures,
        (s) => s.structureType === STRUCTURE_LAB
      )[0];
      if (lab) keepAmount = REACTION_AMOUNT;
      else keepAmount = 0;
    }
    ...
    if (terminal.room.memory.praiseRoom || terminal.room.nukes.length)
      keepAmount = 0;
    // Next resource if we don't have enough to send
    let available = terminal.room.store(resource) - keepAmount;
    if (available > terminal.store[resource])
      available = terminal.store[resource];
    if (available <= keepAmount * 0.1 || available < 100) continue;
    // Find room in need
    let needyTerminal = _.sortBy(
      _.filter(
        Game.structures,
        (r) =>
          r.structureType === STRUCTURE_TERMINAL &&
          !r.room.nukes.length &&
          r.room.name !== terminal.room.name &&
          r.room.store(resource) < keepAmount &&
          !r.room.memory.praiseRoom &&
          r.store.getFreeCapacity()
      ),
      function (s) {
    ...
    if (needyTerminal) {
      ...
      terminal.room.name !== Memory.saleTerminal.room &&
...
function emergencyEnergy(terminal) {
  ...
  if (
    terminal.store[RESOURCE_ENERGY] &&
    !Memory.roomCache[terminal.room.name].requestingSupport &&
    !Memory.roomCache[terminal.room.name].threatLevel &&
    !terminal.room.nukes.length
  ) {
    // Find needy terminals
    ...
    let responseNeeded = _.min(
      _.filter(
        myRooms,
        (r) =>
          r.name !== terminal.room.name &&
          ((Memory.roomCache[r.name] &&
            Memory.roomCache[r.name].threatLevel >= 3) ||
            r.memory.nuke > 1500) &&
          r.terminal &&
          r.energy < ENERGY_AMOUNT * 2
      ),
      ".energy"
    );
    if (responseNeeded && responseNeeded.name) {
      ...
      switch (
        terminal.send(RESOURCE_ENERGY, availableAmount, needyTerminal.room.name)
      ) {
        case OK:
          log.a(
            "Siege Supplies " +
    ...
    Memory.roomCache[terminal.room.name].requestingSupport &&
    terminal.room.energy < ENERGY_AMOUNT * 2 &&
    Game.market.credits >= CREDIT_BUFFER * 0.25
  ) {
    let sellOrder = _.min(
      globalOrders.filter(
        (order) =>
...
function dealFinder(terminal, globalOrders) {
  if (terminal.store.getFreeCapacity() >= TERMINAL_CAPACITY * 0.25) {
    let sellOrder = _.min(
      globalOrders.filter(
        (order) =>
          ...
          order.price <=
            latestMarketHistory(order.resourceType)["avgPrice"] * 0.7 &&
          Game.market.calcTransactionCost(
            order.amount,
            terminal.room.name,
            order.roomName
          ) <
            terminal.store[RESOURCE_ENERGY] * 0.5
      ),
      "price"
    );
    ...
    if (sellOrder.price * buyAmount > spendingMoney)
      buyAmount = _.round(
        buyAmount * (spendingMoney / (sellOrder.price * buyAmount))
      );
    ...
    let buyer = _.max(
      globalOrders.filter(
        (order) =>
          ...
          order.type === ORDER_BUY &&
          order.price >= latestMarketHistory(PIXEL)["avgPrice"] * 0.8
      ),
      "price"
    );
    if (buyer.id) {
...
function profitCheck(force = false) {
                                      ...
                                      if (
                                        force ||
                                        profitTracking.lastData + hourlyTick <
                                          Game.time ||
                                        !profitTracking.lastData
                                      ) {
                                        profitTracking.lastData = Game.time;
                                        //let hourlyProfits = profitTracking.hourArray || [];
                                        let lastCredit =
                                          profitTracking.lastTotalAmount ||
                                          Game.market.credits;
                                        profitTracking.lastTotalAmount =
                                          Game.market.credits;
                                        let hourChange =
                                          Game.market.credits - lastCredit;
                                        // Spending account is capped at 100k
                                        if (
                                          Game.market.credits > 100000 &&
                                          spendingMoney > 100000
                                        ) {
                                          spendingMoney = 100000;
                                          log.w(
                                            "New spending account amount (HOURLY UPDATE) - " +
                                              spendingMoney,
                                            "Market: "
                                          );
                                        }
     ...
     if (hourlyProfits.length < 240) {
            ...
            hourlyProfits.push(hourChange);
                                        ...
                                        if (
                                          Game.market.credits > CREDIT_BUFFER &&
                                          Math.random() > 0.5 &&
                                          spendingMoney < 1000
                                        ) {
                                          let bankersCut =
                                          ...
                                          log.w(
                                            "New spending account amount (RANDOM INFLUX) - " +
                                              spendingMoney,
                                            "Market: "
                                          );
                                        }
