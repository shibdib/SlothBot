 ...
 *
 * CPU Wins:
 * - Structure count caching (big win)
 * - Heavier throttling on rampart/road/minCut calculations
 * - Smarter early exits in all builders
 * - Better PathFinder caching in roadBuilder
 * - Reduced repeated lookFor / filter calls
 *
 * Cleanup:
 * - Consolidated logic
 * - Better comments
 * - Removed dead code paths
 * - Cleaner hub/lab/tower placement
 */
...
const tickTracker = {};
const linkTracker = {};
const extensionPositionCache = {};
const STRUCTURE_COUNT_CACHE = {data: {}, tick: 0};
...
function shouldRunAtAll() {
    return (tickTracker['lastTick'] || 0) < Game.time;
}
...
function getStructureCounts(room) {
    ...
    if (STRUCTURE_COUNT_CACHE.tick !== currentTick) {
        STRUCTURE_COUNT_CACHE.data = {};
        STRUCTURE_COUNT_CACHE.tick = currentTick;
    }
    if (STRUCTURE_COUNT_CACHE.data[room.name]) {
        return STRUCTURE_COUNT_CACHE.data[room.name];
    }
    ...
    STRUCTURE_COUNT_CACHE.data[room.name] = counts;
    return counts;
}
...
function buildMissingStructures(room, level) {
    ...
    if (room.memory.dynamicLayout) skipTypes.push(STRUCTURE_EXTENSION);
...
function buildAuxiliaryStructures(room) {
    if (room.spawns[0]) auxiliaryBuilding(room);
}
...
function buildFromLayout(room, countCheck) {
    ...
    if (room.controller.level === 1 && !initialSpawn) {
        ...
        else if (pos.checkForRampart().hits >= 10000) pos.createConstructionSite(STRUCTURE_SPAWN);
        return;
    } else {
    ...
    if (filter.length) {
        ...
        if (!room.storage && !room.memory.protoStorage && room.controller.level < 4 && room.controller.level > 1) {
            ...
            } else if (pos.checkForAllStructure()?.structureType === STRUCTURE_CONTAINER) {
...
function removeExcessRoads(room) {
    const noRoad = _.filter(room.impassibleStructures, s => s.pos.checkForRoad());
    if (noRoad.length) {
...
function removeBadStructures(room) {
    const badStructure = _.filter(room.structures, s => isBadStructure(s, room));
    if (badStructure.length) badStructure.forEach(s => s.destroy());
}

function isBadStructure(structure, room) {
    if (room.controller.level >= 6) return structure.owner && structure.owner.username !== MY_USERNAME;
    if (room.controller.level >= 4) return structure.owner && structure.owner.username !== MY_USERNAME && structure.structureType !== STRUCTURE_TERMINAL;
    return false;
}
...
function buildSourceExtensions(room) {
    ...
    for (const source of room.sources) {
        ...
        if (!link) {
            const linkSite = container.pos.findInRange(FIND_CONSTRUCTION_SITES, 1).find(s => s.structureType === STRUCTURE_LINK);
        ...
        for (let dx = -1; dx <= 1; dx++) {
            for (let dy = -1; dy <= 1; dy++) {
                ...
                if (pos.checkForWall() || pos.isEqualTo(source.pos) || pos.isEqualTo(link.pos) || (hub && pos.getRangeTo(hub) <= 5)) continue;
...
function linkBuilder(room) {
    ...
    const currentLinks = room.links.length + room.constructionSites.filter(s => s.structureType === STRUCTURE_LINK).length;
    ...
    if (!room.memory.controllerLink || !Game.getObjectById(room.memory.controllerLink)) {
        ...
        if (existingLink) {
            ...
            if (site) return true;
            const zoneTerrain = room.lookForAtArea(LOOK_TERRAIN, room.controller.pos.y - 2, room.controller.pos.x - 2, room.controller.pos.y + 2, room.controller.pos.x + 2, true);
    ...
    if (currentLinks < linkLimit && sortedSources.length > 0 && buildSourceLink(room, sortedSources[0])) return true;
    ...
    if (!room.memory.hubLink || !Game.getObjectById(room.memory.hubLink)) {
        ...
        if (existingLink) room.memory.hubLink = existingLink.id;
        else {
    ...
    if (currentLinks < linkLimit && sortedSources.length > 1 && buildSourceLink(room, sortedSources[1])) return true;

    // Remote Links (RCL 8)
    if (currentLinks < linkLimit) {
        const remoteRooms = _.uniq(_.pluck(_.filter(Game.creeps, c => c.my && c.memory.colony === room.name && c.memory.role === 'remoteHarvester' && c.memory.other?.haulingRequired), 'room.name'));
        for (const remoteRoom of remoteRooms) {
            ...
            for (let xOff = -3; xOff <= 3; xOff++) {
                for (let yOff = -3; yOff <= 3; yOff++) {
                    ...
                    const pos = new RoomPosition(startPos.x + xOff, startPos.y + yOff, room.name);
    ...
    function buildSourceLink(room, source) {
        ...
        if (site) return true;
        const zoneTerrain = room.lookForAtArea(LOOK_TERRAIN, sourceContainer.pos.y - 1, sourceContainer.pos.x - 1, sourceContainer.pos.y + 1, sourceContainer.pos.x + 1, true);
        for (let key in zoneTerrain) {
            const position = new RoomPosition(zoneTerrain[key].x, zoneTerrain[key].y, room.name);
...
function sourceBuilder(room) {
    ...
    function buildSourceContainer(source, room) {
        ...
        if (!sourceContainer) {
            ...
            const sourceBuild = _.find(source.pos.findInRange(FIND_CONSTRUCTION_SITES, 1), s => s.structureType === STRUCTURE_CONTAINER);
            if (!sourceBuild) {
            ...
            if (!source.memory.distanceToHub) source.memory.distanceToHub = source.pos.findPathTo(room.hub).length;
...
function controllerBuilder(room) {
    ...
    if (!controllerContainer && room.level >= 2 && !controllerLink) {
        controllerContainer = room.controller.pos.findInRange(room.containers, 3, {
            filter: s => !s.pos.isNearTo(s.pos.findClosestByRange(FIND_SOURCES)) && !s.pos.isNearTo(s.pos.findClosestByRange(FIND_MINERALS))
        })[0];
        if (!controllerContainer) {
            const controllerBuild = room.controller.pos.findInRange(FIND_CONSTRUCTION_SITES, 3, {filter: s => s.structureType === STRUCTURE_CONTAINER})[0];
            if (!controllerBuild) {
                let possibles = [];
                    ...
                    for (let xOff = -1; xOff <= 1; xOff++) {
                        for (let yOff = -1; yOff <= 1; yOff++) {
                            if (xOff !== 0 || yOff !== 0) {
                                ...
                                if (pos.getRangeTo(room.controller) <= 2 && !pos.checkForImpassible() && !pos.checkIfOutOfBounds() && !pos.isNearTo(pos.findClosestByRange(FIND_SOURCES)) && !pos.isNearTo(pos.findClosestByRange(FIND_MINERALS))) {
                    ...
                    for (let xOff = -2; xOff <= 2; xOff++) {
                        for (let yOff = -2; yOff <= 2; yOff++) {
                            if (xOff !== 0 || yOff !== 0) {
                                ...
                                if (!pos.checkForImpassible() && !pos.checkIfOutOfBounds() && !pos.isNearTo(pos.findClosestByRange(FIND_SOURCES)) && !pos.isNearTo(pos.findClosestByRange(FIND_MINERALS))) {
                                    possibles.push({x: pos.x, y: pos.y});
                                }
                            }
                        }
                ...
                const closestPos = getClosestPosition(possibles, room.hub);
                if (closestPos) if (closestPos.createConstructionSite(STRUCTURE_CONTAINER) === OK) return true;
            }
        } else {
    ...
    function getClosestPosition(positions, hub) {
        let closestPos = null, closestRange = Infinity;
...
function rampartBuilder(room, layout = undefined, count = false) {
    ...
    if (room.level >= 8 && buildQuadTraps(room)) return true;
    ...
    function buildProtectiveRamparts(room) {
        ...
        const vulnerableStructures = room.structures.filter(s => s.structureType !== STRUCTURE_ROAD && s.structureType !== STRUCTURE_WALL && !s.pos.checkForRampart() && !s.pos.checkForConstructionSites());
        ...
        if (room.level >= SPECIAL_RAMPARTS) {
            if (PROTECT_SOURCES) for (let source of room.sources) {
                if (counter >= 3) return true;
                if (buildRampartAround(source.pos)) counter++;
            }
            if (PROTECT_MINERAL && counter < 3 && buildRampartAround(room.mineral.pos)) counter++;
            if (PROTECT_CONTROLLER && counter < 3 && buildRampartAround(room.controller.pos)) counter++;
            if (PROTECT_STRUCTURES && room.level >= 8) {
                for (let structure of room.structures) {
                    if (counter >= 3) return true;
                    if (protectedStructureTypes.includes(structure.structureType) && !structure.pos.checkForRampart() && !structure.pos.checkForConstructionSites()) {
                        if (structure.pos.createConstructionSite(STRUCTURE_RAMPART) === OK) counter++;
                    }
                }
            }
    ...
    function buildQuadTraps(room) {
        ...
        for (const trap of quadTraps[room.name]) {
            ...
            if (pos.isNearTo(room.controller) || pos.isNearTo(room.mineral) || room.sources.some(s => pos.isNearTo(s))) continue;
    ...
    function initializeRampartSpots(room, layout, count) {
        ...
        if (count && ROOM_RAMPART_SPOTS[room.name]) return _.size(JSON.parse(ROOM_RAMPART_SPOTS[room.name]));
    }
    ...
    function placeRamparts(room) {
        ...
        if (!spots || !spots.length) addExistingRampartsToSpots(room, spots);
    ...
    function shouldBuildRampartAtPosition(pos, room) {
        if (isNearProtectedStructure(pos, room)) return !pos.checkForRampart() && !pos.checkForConstructionSites();
        if (pos.checkForWall() && pos.checkForRoad()) return handleTunnelRampart(pos);
        ...
        for (let xOff = -1; xOff <= 1; xOff++) {
            for (let yOff = -1; yOff <= 1; yOff++) {
                if (xOff !== 0 || yOff !== 0) {
                    ...
                    if (!newPos.checkForWall() && !newPos.checkForBarrierStructure() && !newPos.checkForConstructionSites() && newPos.createConstructionSite(STRUCTURE_RAMPART) === OK) return true;
                }
            }
        }
        return false;
...
function roadBuilder(room, layout) {
    ...
    for (let container of sourceContainers) if (buildRoadFromTo(room, spawn, container)) return true;

    let controllerContainer = Game.getObjectById(room.memory.controllerContainer);
    if (controllerContainer && buildRoadFromTo(room, spawn, controllerContainer)) return true;
    ...
    function buildRoadToNeighborExits(spawn, room) {
        ...
        for (let direction in directionToExit) {
            if (neighboring[direction]) {
                ...
                if (buildRoadFromTo(spawn.room, spawn, exits[middle])) return true;
            }
        }
        return false;
    }
    ...
    function buildTowerRoads(room) {
        ...
        for (const tower of towers) if (buildRoadFromTo(room, spawn, tower)) return true;
        return false;
    }
    ...
    function buildMineralLinkAndLabRoads(room) {
        ...
        if (labsLinks.length) {
            ...
            for (let lab of labsLinks) if (buildRoadFromTo(room, lab, hub)) return true;
        }
        return false;
    }
    ...
    function buildRoadFromTo(room, start, end) {
        ...
        let points;
        if (cached) points = JSON.parse(cached.path);
        else {
        ...
        if (ROAD_CACHE[room.name] && ROAD_CACHE[room.name][key]) ROAD_CACHE[room.name][key].complete = true;
    }
    ...
    function buildCostMatrix(roomName) {
        ...
        for (let y = 0; y < 50; y++) {
            for (let x = 0; x < 50; x++) {
                ...
                if (tile === TERRAIN_MASK_WALL) costMatrix.set(x, y, Infinity);
                else if (tile === TERRAIN_MASK_SWAMP) costMatrix.set(x, y, 60);
                else costMatrix.set(x, y, 20);
            }
        }
        let room = Game.rooms[roomName];
        if (room) {
            room.structures.forEach(structure => {
                if (structure.structureType === STRUCTURE_ROAD) costMatrix.set(structure.pos.x, structure.pos.y, 1);
                else if (structure.structureType === STRUCTURE_CONTAINER) costMatrix.set(structure.pos.x, structure.pos.y, 100);
                else if (_.includes(OBSTACLE_OBJECT_TYPES, structure.structureType)) costMatrix.set(structure.pos.x, structure.pos.y, Infinity);
            });
            room.constructionSites.forEach(site => {
                if (site.structureType === STRUCTURE_ROAD) costMatrix.set(site.pos.x, site.pos.y, 1);
            });
        }
        return costMatrix;
    }
    ...
    function buildRoad(pos) {
        if (pos.checkForRoad() || pos.checkForConstructionSites() || pos.checkForImpassible() || pos.checkForWall()) return false;
    ...
    function removeRedundantRoads(room, layout) {
        ...
        if (cache) {
            for (const key in cache) {
                for (const point of JSON.parse(cache[key].path)) needed.add(`${point.x}x${point.y}`);
            }
        } else return false;
...
function findHub(room, hubCheck = undefined) {
    if (room.controller.owner && room.controller.owner.username === MY_USERNAME && room.memory.bunkerHub && room.memory.bunkerHub.x && room.memory.bunkerHub.y) return true;
...
function findLabHub(room) {
    ...
    primary:
        for (let i = 1; i < 500;) {
            ...
            for (let pos of labTemplate) {
                let structure = {x: x + pos.x, y: y + pos.y};
                ...
                if (structurePos.checkIfOutOfBounds() || structurePos.checkForImpassible() || structurePos.getRangeTo(roomHub) <= 6 || structurePos.isNearTo(room.controller) || structurePos.isNearTo(structurePos.findClosestByRange(FIND_SOURCES)) || structurePos.countOpenTerrainAround() < 7) continue primary;
            }
    ...
    if (complete && _.size(storedLabPossibles[room.name])) {
        let choice = _.min(storedLabPossibles[room.name], p => new RoomPosition(p.x, p.y, room.name).getRangeTo(new RoomPosition(room.memory.bunkerHub.x, room.memory.bunkerHub.y, room.name)));
        room.memory.labHub = {x: choice.x, y: choice.y};
...
function buildTowersFromHubs(room) {
    ...
    const current = room.towers.length + room.constructionSites.filter(s => s.structureType === STRUCTURE_TOWER).length;
...
function getUndefendedExits(roomName) {
    ...
    for (const dir in dirToFind) {
        ...
        if (Object.keys(Game.map.describeExits(neighbour) || {}).length <= 1) undefended.push(dirToFind[dir]);
    }
    return undefended;
}
...
function findTowerHub(room) {
    ...
    for (const dir in dirToFind) {
        if (!neighboring[dir] || undefendedExits.includes(dirToFind[dir])) continue;
        const exits = room.find(dirToFind[dir]);
        if (!exits.length) continue;
        allExitTiles.push(...exits);
        threatPoints.push(exits[Math.floor(exits.length * 0.25)], exits[Math.floor(exits.length * 0.5)], exits[Math.floor(exits.length * 0.75)]);
    }
    ...
    for (let x = 4; x <= 45; x++) {
        for (let y = 4; y <= 45; y++) {
            ...
            if (hubDist < 6 || hubDist > 23) continue;
...
function isValidRampartPosition(position) {
    return !position.checkForWall() && !position.checkForConstructionSites() && !position.checkForRampart();
}

function cacheRoad(room, from, to, path) {
    let key = getPathKey(from, to);
    let cache = ROAD_CACHE[room.name] || {};
    cache[key] = {path: JSON.stringify(path), tick: Game.time};
...
let protectedStructureTypes = [STRUCTURE_SPAWN, STRUCTURE_STORAGE, STRUCTURE_TOWER, STRUCTURE_POWER_SPAWN, STRUCTURE_TERMINAL, STRUCTURE_NUKER, STRUCTURE_OBSERVER, STRUCTURE_LINK, STRUCTURE_EXTRACTOR, STRUCTURE_LAB, STRUCTURE_CONTAINER];

let bunkerTemplate = [ /* ... same as original ... */];
let labTemplate = [{"x": 0, "y": 0}, {"x": 0, "y": 1}, {"x": 1, "y": 0}, {"x": -1, "y": 0}, {"x": 0, "y": -1}, {
    "x": 1,
    "y": -1
}, {"x": 1, "y": 1}, {"x": 0, "y": 2}, {"x": -1, "y": 1}, {"x": -1, "y": 2}];
const coreTemplate = [ /* ... same as original ... */];
...
function generateExtensionPositions(room) {
    ...
    for (const entry of coreTemplate) for (const {x, y} of entry.pos) excluded.add(`${hub.x + x},${hub.y + y}`);
...
function placeExtensionsDynamically(room) {
    ...
    const existing = room.extensions.length + room.constructionSites.filter(s => s.structureType === STRUCTURE_EXTENSION).length;
    ...
    for (const {x, y} of positions) {
        ...
        if (result === ERR_FULL || result === ERR_RCL_NOT_ENOUGH) return false;
    }
    return false;
}