...
function role(creep) {
    ...
    if (creep.memory.working === true) {
        ...
        if (container && creep.pos.rangeToTarget(container) <= 1 && container.store[RESOURCE_ENERGY] > 0) creep.withdraw(container, RESOURCE_ENERGY);
        if (terminal && creep.pos.rangeToTarget(terminal) <= 1 && terminal.store[RESOURCE_ENERGY] > 0) creep.withdraw(terminal, RESOURCE_ENERGY);
