...
module.exports.linkControl = function (room) {
    let links = shuffle(_.filter(room.structures, (s) => s.structureType === STRUCTURE_LINK && !s.cooldown && s.energy >= 50 && s.id !== s.room.memory.controllerLink && s.id !== s.room.memory.hubLink));
    ...
    if (hubLink && controllerLink && hubLink.energy > 100 && controllerLink.energy < 50) {
