    ...
    if (this.memory.role === 'longbow') {
        if (Memory.targetRooms[this.memory.targetRoom] && Memory.targetRooms[this.memory.targetRoom].level > 1) {
            ...
            if (this.memory.squadLeader && !this.handleMilitaryCreep(false, false)) {
                let squadMember = _.filter(Game.creeps, (c) => c.memory && c.memory.targetRoom === this.memory.targetRoom && c.memory.operation === 'hold' && !c.memory.squadLeader && c.memory.role === 'longbow');
                ...
                if (Memory.targetRooms[this.room.name]) Memory.targetRooms[this.room.name].unClaimer = !this.room.controller.upgradeBlocked;
