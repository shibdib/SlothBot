...
Creep.prototype.getEnergy = function (range = 250, hauler = false) {
    ...
    //Storage
    let sStorage = _.filter(this.room.structures, (s) => s.structureType === STRUCTURE_STORAGE)[0];
    if ((sStorage && sStorage.store[RESOURCE_ENERGY] > ENERGY_AMOUNT && sStorage.pos.rangeToTarget(this) <= range && sStorage.pos.rangeToTarget(this) > 1) || (sStorage && this.room.memory.responseNeeded)) {
        let weight = 0.3;
