...
let globals = function () {
                            ...
                            try {
                              ...
                              console.log(
                                "Loaded config for " + Game.shard.name
                              );
                            } catch (e) {
                              try {
                                require(Memory.customConfig);
                                console.log(
                                  "Loaded config for " + Memory.customConfig
                                );
                              } catch (e) {
                                require("config");
                                console.log(
                                  "No custom config found loading config.js"
                                );
                              }
                            }
                            ...
                            global.UNIT_COST = (body) =>
                              _.sum(body, (p) => BODYPART_COST[p.type || p]);
                            ...
                            global.EST_TICKS_PER_MIN = Math.ceil(
                              60 / EST_SEC_PER_TICK
                            ); // 60s
                            global.EST_TICKS_PER_DAY = Math.ceil(
                              86400 / EST_SEC_PER_TICK
                            ); // 24h * 60m * 60s = 86400s

                            global.toStr = (obj) =>
                              JSON.stringify(obj, null, 2); // shortcut to stringify an object (idea credit: warinternal, from the Screeps Slack)
     ...
     */
                            global.defineCachedGetter = function (
                              proto,
                              propertyName,
                              fn
                            ) {
                            ...
                            global.populateLOANlist = function (
                              ...
                              if (
                                typeof RawMemory.setActiveForeignSegment ==
                                  "function" &&
                                !!~[
                                  "shard0",
                                  "shard1",
                                ...
                                if (
                                  ...
                                  global.LOANlist = [];
                                  if (typeof Memory.LOANalliance == "undefined")
                                    Memory.LOANalliance = "";
                                }
                                ...
                                if (
                                  ...
                                  typeof RawMemory.foreignSegment !=
                                    "undefined" &&
                                  RawMemory.foreignSegment.username ==
                                    LOANuser &&
                                  ...
                                  if (RawMemory.foreignSegment.data == null) {
                                    ...
                                    let allMyRooms = _.filter(
                                      Game.rooms,
                                      (aRoom) =>
                                        typeof aRoom.controller !=
                                          "undefined" && aRoom.controller.my
                                    );
                                    if (allMyRooms.length == 0) {
                                      let allMyCreeps = _.filter(
                                        Game.creeps,
                                        (creep) => true
                                      );
                                      if (allMyCreeps.length == 0) {
                                        ...
                                        myUsername =
                                          allMyCreeps[0].owner.username;
                                    } else
                                      myUsername =
                                        allMyRooms[0].controller.owner.username;
                                    for (
                                      let iL = LOANdataKeys.length - 1;
                                      iL >= 0;
                                      iL--
                                    ) {
                                      if (
                                        LOANdata[LOANdataKeys[iL]].indexOf(
                                          myUsername
                                        ) >= 0
                                      ) {
                                        ...
                                        global.LOANlist =
                                          LOANdata[LOANdataKeys[iL]];
                                        global.LOANlist = global.LOANlist.filter(
                                          function (uname) {
                                            return disavowed.indexOf(uname) < 0;
                                          }
                                        );
                                        global.LOANlist.concat(MANUAL_FRIENDS);
                                        Memory.LOANalliance = LOANdataKeys[
                                          iL
                                        ].toString();
                                        return true;
                                      }
                            ...
                            global.getLevel = function (room) {
                              ...
                              if (energyLevel <= room.controller.level)
                                return energyLevel;
                              else return room.controller.level;
                            };

                            global.roomLink = function (
                              roomArg,
                              text = undefined,
                              select = true
                            ) {
