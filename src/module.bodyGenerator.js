...
module.exports.bodyGenerator = function (level, role, room = undefined, misc = undefined) {
    ...
    switch (role) {
      ...
      case "drone":
        ...
        work =
          _.floor(
            (room.energyCapacityAvailable * _.random(0.1, 0.3)) /
              BODYPART_COST[WORK]
          ) || 1;
        if (work > 15) work = 15;
        carry =
          _.floor(
            (room.energyCapacityAvailable * _.random(0.05, 0.2)) /
              BODYPART_COST[CARRY]
          ) || 1;
      ...
      case "waller":
        ...
        work =
          _.floor(
            (room.energyCapacityAvailable * _.random(0.2, 0.5)) /
              BODYPART_COST[WORK]
          ) || 1;
        if (work > 15) work = 15;
        carry =
      ...
      case "praiseUpgrader":
        ...
        work =
          _.floor(
            (room.energyCapacityAvailable * 0.7 - 100) / BODYPART_COST[WORK]
          ) || 1;
        if (work > 10) work = 10;
        carry = 1;
        move = 1;
      ...
      case "upgrader":
        ...
        if (level !== room.controller.level || room.nukes.length) {
          ...
          work =
            _.floor(
              (room.energyCapacityAvailable * _.random(0.3, 0.5)) /
                BODYPART_COST[WORK]
            ) || 1;
          carry = 1;
          move = work + carry;
          ...
          work =
            _.floor(
              (room.energyCapacityAvailable * _.random(0.5, 0.7)) /
                BODYPART_COST[WORK]
            ) || 1;
          if (work > 48) work = 48;
          if (level === 8 && room.energyState) work = 15;
          else if (level === 8) work = 1;
          //work *= deficit;
          move = 1;
          carry = 1;
          break;
      ...
      case "expediter":
        ...
        carry =
          _.floor(
            (room.energyCapacityAvailable * 0.25) / BODYPART_COST[CARRY]
          ) || 1;
      ...
      case "stationaryHarvester":
        ...
        work =
          _.floor(
            ((room.energyCapacityAvailable - 50) * 0.5) / BODYPART_COST[WORK]
          ) || 1;
        // 6 Is the cap
        if (work > 6) work = 6;
        carry = 1;
      ...
      case "mineralHarvester":
        work =
          _.floor(
            (room.energyCapacityAvailable * 0.75) / BODYPART_COST[WORK]
          ) || 1;
        // 50 Is the cap
        if (work > 50) work = 50;
        break;
      ...
      case "attacker":
        tough =
          _.floor(
            (room.energyCapacityAvailable * 0.15) / BODYPART_COST[TOUGH]
          ) || 1;
        if (tough > 7) tough = 7;
        attack =
          _.floor(
            (room.energyCapacityAvailable * 0.35) / BODYPART_COST[ATTACK]
          ) || 1;
      ...
      case "drainer":
        ...
        tough =
          _.floor(
            (room.energyCapacityAvailable * 0.1) / BODYPART_COST[TOUGH]
          ) || 1;
        if (tough > 7) tough = 7;
        heal =
          _.floor((room.energyCapacityAvailable * 0.4) / BODYPART_COST[HEAL]) ||
          1;
        if (heal > 17) heal = 17;
      ...
      case "defender":
        deficitExemption = true;
        if (Math.random() > 0.49 || level < 5)
          attack =
            _.floor(
              (room.energyCapacityAvailable * 0.75) / BODYPART_COST[ATTACK]
            ) || 1;
        else
          rangedAttack =
            _.floor(
        ...
        if (attack > 45) attack = 45;
        else if (rangedAttack > 45) rangedAttack = 45;
        move =
          _.floor((room.energyCapacityAvailable * 0.2) / BODYPART_COST[MOVE]) ||
          1;
        if (move > 5) move = 5;
        break;
      case "longbow":
        ...
        rangedAttack =
          _.floor(
            (room.energyCapacityAvailable * 0.35) / BODYPART_COST[RANGED_ATTACK]
          ) || 1;
        if (rangedAttack > 19) rangedAttack = 19;
        heal = _.floor(
          (room.energyCapacityAvailable * 0.1) / BODYPART_COST[HEAL]
        );
        if (heal > 6) heal = 6;
      ...
      case "poke":
        deficitExemption = true;
        if (Math.random() > 0.5) rangedAttack = 1;
        else attack = 1;
        move = 1;
        break;
      case "deconstructor":
      ...
      case "siegeEngine":
        ...
        attack =
          _.floor(
            (room.energyCapacityAvailable * 0.3) / BODYPART_COST[ATTACK]
          ) || 1;
        if (attack > 15) attack = 15;
        rangedAttack = _.floor(
          (room.energyCapacityAvailable * 0.05) / BODYPART_COST[RANGED_ATTACK]
        );
      ...
      case "claimAttacker":
        claim =
          _.floor(
            (room.energyCapacityAvailable * 0.4) / BODYPART_COST[CLAIM]
          ) || 1;
      ...
      case "reserver":
        claim =
          _.floor(
            (room.energyCapacityAvailable * 0.5) / BODYPART_COST[CLAIM]
          ) || 1;
        if (claim > 25) claim = 25;
        if (level >= 7) move = claim * 0.5;
        else move = claim;
        break;
      case "fuelTruck":
        carry = 20;
      ...
      case "remoteHarvester":
        deficitExemption = true;
        work =
          _.floor((room.energyCapacityAvailable * 0.4) / BODYPART_COST[WORK]) ||
          1;
        if (work > 10) work = 10;
        if (!room.energyState && work > 5) work = 5;
        carry = 1;
        if (room.memory.roadsBuilt || level >= 6) move = work / 2;
        else move = work;
        break;
      case "remoteHauler":
        deficitExemption = true;
        carry =
          _.floor(
            (room.energyCapacityAvailable * 0.4) / BODYPART_COST[CARRY]
          ) || 1;
        if (level <= 6 && carry > 24) carry = 24;
        else if (level > 6 && carry > 30) carry = 30;
        if (!room.energyState && carry > 12) carry = 12;
        if (Math.random() > 0.7) work = 1;
        else work = 0;
        if (level < 6) move = carry + work;
        else move = _.ceil((carry + work) / 2);
        break;
      case "roadBuilder":
        work =
          _.floor(
      ...
      case "commodityMiner":
        work =
          _.floor(
            (room.energyCapacityAvailable * 0.45) / BODYPART_COST[WORK]
          ) || 1;
        if (work > 20) work = 20;
        carry = 1;
        move = work + carry;
