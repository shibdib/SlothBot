...
function shibMove(creep, heading, options = {}) {
    ...
    if (pathInfo.pathPosTime && pathInfo.pathPosTime >= STATE_STUCK) {
        let bumpCreep = _.filter(creep.room.creeps, (c) => c.memory && c.getActiveBodyparts(MOVE) && c.pos.x === pathInfo.newPos.x && c.pos.y === pathInfo.newPos.y && (!c.memory._shibMove || !c.memory._shibMove.path) &&
            c.memory.role !== 'stationaryHarvester' && (c.memory.role !== 'upgrader' || (creep.memory.role !== 'upgrader' && c.memory.role === 'upgrader')) && c.memory.role !== 'reserver' && c.memory.role !== 'remoteHarvester')[0];
        if (bumpCreep) {
...
function cachePath(creep, from, to, path) {
    ...
    if (creep.memory.localCache) creep.memory.localPathCache = cache; else pathCache = cache;
}
