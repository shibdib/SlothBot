...
Creep.prototype.marauding = function () {
    ...
    if (!this.memory.other.destination) {
        ...
        let target = _.sortBy(_.filter(filtered, (r) => _.includes(Memory._badBoyArray, r.user) && r.level && !r.towers), function (r) {
            Game.map.getRoomLinearDistance(roomName, r.name);
        })[0] || _.sortBy(_.filter(filtered, (r) => _.includes(Memory._badBoyArray, r.user) && !r.level), function (r) {
            Game.map.getRoomLinearDistance(roomName, r.name);
        })[0] || _.sortBy(_.filter(filtered, (r) => Memory.ncpArray && _.includes(Memory.ncpArray, r.user) && !r.level), function (r) {
            Game.map.getRoomLinearDistance(roomName, r.name);
        })[0] || _.sortBy(_.filter(filtered, (r) => !_.includes(FRIENDLIES, r.user) && r.level && !r.towers && NEW_SPAWN_DENIAL), function (r) {
            Game.map.getRoomLinearDistance(roomName, r.name);
        })[0] || _.sortBy(_.filter(filtered, (r) => !_.includes(FRIENDLIES, r.user) && !r.level), function (r) {
            Game.map.getRoomLinearDistance(roomName, r.name);
        })[0];
        if (target) {
