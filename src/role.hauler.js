...
function role(creep) {
    ...
    if (!creep.getSafe(true)) {
        if (Game.time % 15 === 0 || creep.memory.labTech || creep.memory.nuclearEngineer || creep.memory.terminalWorker) if (terminalWorker(creep) || boostDelivery(creep) || nuclearEngineer(creep)) return null;
...
function terminalWorker(creep) {
    ...
    if (creep.memory.terminalDelivery) {
        ...
        for (let resourceType in storage.store) {
            ...
            if (_.includes(END_GAME_BOOSTS, resourceType) || _.includes(TIER_2_BOOSTS, resourceType) || _.includes(TIER_1_BOOSTS, resourceType)) {
