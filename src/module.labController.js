...
function manageActiveLabs(room) {
    ...
    if (room.store(room.memory.producingBoost) > cutOff) {
    ...
    let secondaryLabs = _.filter(room.structures, (s) => s.structureType === STRUCTURE_LAB && !primaryLabs[room.name].includes(s.id) && !s.cooldown &&
...
function manageBoostProduction(room) {
    ...
    if (!primaryLabs[room.name]) {
        ...
        let labs = _.filter(room.structures, (s) => s.structureType === STRUCTURE_LAB && ((s.pos.x === labHub.x && s.pos.y === labHub.y) || (s.pos.x === labHub.x && s.pos.y === labHub.y + 1)));
    ...
    let secondaryLabs = _.filter(room.structures, (s) => s.structureType === STRUCTURE_LAB && !primaryLabs[room.name].includes(s.id));
