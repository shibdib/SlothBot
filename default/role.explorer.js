...
module.exports.role = function (creep) {
    ...
    if (!creep.memory.destination) {
        ...
        if (!creep.memory.usedPortal && portal && (creep.memory.other.portalJump || Math.random() > 0.01)) {
            ...
            let adjacent = _.filter(_.map(Game.map.describeExits(creep.pos.roomName)), (r) => roomStatus(r) === roomStatus(creep.memory.overlord) && (!INTEL[creep.room.name].obstacles || creep.pos.findPathTo(Game.map.findExit(this.name, r)).length));
