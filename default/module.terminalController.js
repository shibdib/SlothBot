...
module.exports.terminalControl = function (room) {
    ...
    // Clear saleTerminal room if an incoming nuke is detected
    ...
    if (!Memory.saleTerminal.room || Memory.saleTerminal.saleSet + 15000 < Game.time || !Game.rooms[Memory.saleTerminal.room]) {
        // Clear invalid saleTerminal if conditions are not met
        ...
        if (roomData.controller.level === MAX_LEVEL) {
        ...
        const availableRooms = _.filter(Game.structures, s =>
            ...
            s.room.controller.level === MAX_LEVEL &&
            !s.room.nukes.length &&
            s.isActive() &&
    ...
    if (room.name === Memory.saleTerminal.room) {
        ...
        // Handle buying resources if spending money is available
        if (spendingMoney > 0) {
            // Future buy logic can go here (commented out code for now)
...
function pricingUpdate(globalOrders, myOrders) {
    for (let key in myOrders) {
        ...
        let newPrice = calculateNewPrice(order, globalOrders);
...
// Helper function to calculate the new price based on the market conditions
    function calculateNewPrice(order, globalOrders) {
        let newPrice = order.price;
        let competitorOrder;
        let marketHistory = latestMarketHistory(order.resourceType);

        // Track market trend (volatility)
        let marketTrend = getMarketTrend(order.resourceType);

        if (order.type === ORDER_SELL) {
            // Selling strategy: attempt to sell based on market trend and competitor prices
            if (marketHistory) {
                newPrice = adjustForTrend(marketHistory.avg, marketTrend, true);
            } else {
                competitorOrder = _.min(globalOrders.filter(o => o.resourceType === order.resourceType && o.type === ORDER_SELL && !_.includes(MY_ROOMS, o.roomName)), 'price');
                if (competitorOrder && competitorOrder.id) {
                    newPrice = competitorOrder.price - 0.001; // Slightly undercut the lowest competitor
                }
            }
        } else if (order.type === ORDER_BUY) {
            // Buying strategy: attempt to buy slightly above competitors or based on market history, considering market trend
            if (marketHistory) {
                newPrice = adjustForTrend(marketHistory.avg, marketTrend, false);
            }
            competitorOrder = _.max(globalOrders.filter(o => o.resourceType === order.resourceType && o.type === ORDER_BUY && !_.includes(MY_ROOMS, o.roomName)), 'price');
            if (competitorOrder && competitorOrder.id) {
                newPrice = competitorOrder.price + 0.001; // Slightly raise the price to outbid competitors
            } else if (marketHistory) {
                newPrice = marketHistory.avg; // Default to the average market price if no competitor
            }

            // Ensure we don't go above the average price
            if (marketHistory && newPrice > marketHistory.avg) {
                newPrice = marketHistory.avg;
            }
        }
...
// Adjust the price based on market trend
    function adjustForTrend(avgPrice, marketTrend, isSelling) {
        if (marketTrend > 0 && isSelling) {
            // If prices are rising, try to push for a higher price
            return avgPrice * 1.02; // Increase by 2%
        } else if (marketTrend < 0 && isSelling) {
            // If prices are falling, adjust to avoid being undercut
            return avgPrice * 0.98; // Decrease by 2%
        } else if (marketTrend > 0 && !isSelling) {
            // If buying and prices are rising, be more conservative with the price
            return avgPrice * 0.98; // Decrease by 2% for buy orders
        } else if (marketTrend < 0 && !isSelling) {
            // If buying and prices are falling, try to buy slightly above the average
            return avgPrice * 1.02; // Increase by 2% for buy orders
        }
        return avgPrice; // No change if trend is neutral or unknown
    }

// Function to determine market trend (up or down) based on past prices
    function getMarketTrend(resourceType) {
        let history = Game.market.getHistory(resourceType);
        if (!history[0] || history.length < 4) return 0; // No trend if there's insufficient history

        let recentPrice = history[history.length - 1].avgPrice;
        let prevPrice = history[history.length - 2].avgPrice;

        // Calculate trend (positive for up, negative for down, 0 for no change)
        if (recentPrice > prevPrice) {
            return 1; // Price is rising
        } else if (recentPrice < prevPrice) {
            return -1; // Price is falling
        }
        return 0; // No trend
    }
}
...
function placeSellOrders(terminal, globalOrders, myOrders) {
    ...
    for (let resourceType of Object.keys(terminal.store)) {
        ...
        let price = getPriceForResource(resourceType, globalOrders);
...
// Helper function to determine if the sell order is profitable after considering the market fee
...
// Helper function to calculate the sell amount based on available resources
...
// Handle regional commodities with custom logic
...
// Helper function to check if there are existing sell orders for the resource in the current room
...
// Helper function to get the price for a resource type
    function getPriceForResource(resourceType, globalOrders) {
        let price = 5;  // Default price
        let marketHistory = latestMarketHistory(resourceType);
        ...
        if (isLowSupply(resourceType)) {
            price *= 1.05;  // Increase price by 5% if supply is low
        } else if (isHighDemand(resourceType)) {
...
// Helper function to check if supply is low for a given resource
    function isLowSupply(resourceType) {
...
// Helper function to check if demand is high for a given resource
    function isHighDemand(resourceType) {
...
// Helper function to create a sell order
...
function placeBuyOrders(terminal, globalOrders, myOrders) {
    ...
    function getOrderPrice(mineral, isEnergy = false) {
        let price = 0.5;
        let averagePrice;
        const orderType = isEnergy ? RESOURCE_ENERGY : mineral;

        // Get the latest market history and competitor order
        const history = latestMarketHistory(orderType);
    ...
    for (let mineral of shuffle(BASE_MINERALS)) {
        ...
        if (stored < target) {
            ...
            if (['shard0', 'shard1', 'shard2', 'shard3'].includes(Game.shard.name)) {
                ...
                price = getOrderPrice(mineral);
                buyAmount = Math.min(buyAmount, tradeAmount);
                ...
                if (sellOrder && sellOrder.price * buyAmount > spendingMoney) buyAmount = _.floor(spendingMoney / sellOrder.price);
    ...
    if (Game.market.credits > BUY_ENERGY_CREDIT_BUFFER) {
        ...
        if (BUY_ENERGY && !_.find(MY_ROOMS, (r) => Game.rooms[r].terminal && Game.rooms[r].energyState > 1)) {
            if (!_.find(myOrders, (o) => o.resourceType === RESOURCE_ENERGY && o.roomName === terminal.room.name)) {
                price = getOrderPrice(RESOURCE_ENERGY, true);
...
function balanceResources(terminal) {
    ...
    for (let resource of sortedKeys) {
        ...
        if (needyTerminal) {
            if (sendResource(terminal, resource, available, needyTerminal)) return true;
            ...
            if (sendResource(terminal, resource, available, Memory.saleTerminal.room)) return true;
        }
    }
    return false;
    ...
    function determineKeepAmount(resource) {
        ...
        return reactionAmount; // Default reaction amount
    }
    ...
    function findNeedyTerminal(terminal, resource, available) {
        ...
        if (terminal.room.energyState && !needyTerminal) {
...
function dealFinder(terminal, globalOrders) {
    // Set dynamic flip margins based on resource type or market conditions
...
// Set minimum profit margin based on resource and volume
    ...
    let maxSpendAmount = spendingMoney * 0.1;
    ...
    log.w(`DEAL DEAL DEAL: Buying ${buyAmount} ${sellOrder.resourceType} for ${sellOrder.price} credits. Estimated profit: ${potentialProfit} credits. Average price: ${latestMarketHistory(sellOrder.resourceType).avgPrice}`, "Market: ");
    ...
    if (Game.market.deal(sellOrder.id, buyAmount, terminal.pos.roomName) === OK) {
        ...
        spendingMoney -= (sellOrder.price * buyAmount);  // Deduct from the spending money account
        log.w(`Remaining spending account: ${spendingMoney}`, "Market: ");
            ...
            if (transactionCostForSale <= terminal.store[RESOURCE_ENERGY]) {
                switch (Game.market.deal(sellOrderForProfit.id, sellAmount, terminal.pos.roomName)) {
                    case OK:
                        ...
                        spendingMoney += sellAmount * sellOrderForProfit.price;
                        log.w(`Updated spending account: ${spendingMoney}`, "Market: ");
                        return true;
                }
            }
        }
...
function sellPixels(globalOrders) {
    ...
    if (sellAmount >= 10) {
        ...
        if (orderResult === OK) {
            ...
            spendingMoney += sellPrice * sellAmount;  // Update the spending account
            log.w("New spending account amount - " + spendingMoney, "Market: ");
            return true;
        } else {
...
function latestMarketHistory(resource) {
    ...
    // Calculate the sum of avgPrice values using reduce
    let sumAvgPrice = 0;
    for (let entry of history) {
        sumAvgPrice += entry.avgPrice;
    }

    // Calculate the average price
    let avgPrice = (sumAvgPrice / history.length).toFixed(3);  // Round to 3 decimal places

    // Optionally, calculate more statistics like the highest and lowest price in the history
    let highestPrice = Math.max(...history.map(entry => entry.avgPrice));
    let lowestPrice = Math.min(...history.map(entry => entry.avgPrice));

    // Return an object with the average price, highest and lowest prices for better market insight
    return {
        avg: avgPrice,
...
function profitCheck(force = false) {
    ...
    if (force || profitTracking.lastData + hourlyTick < Game.time || !profitTracking.lastData) {
        ...
        const hourChange = Game.market.credits - lastCredit;

        // Private server handling (excluding shard0-3)
        if (!['shard0', 'shard1', 'shard2', 'shard3'].includes(Game.shard.name)) {
            // Reinvest credits more aggressively when we're not on the public shard
            spendingMoney = Math.max(Game.market.credits - CREDIT_BUFFER, 0); // Ensure spendingMoney is not negative
        ...
        // Add 90% of the profit to spending account for the hour to aggressively reinvest
            ...
            spendingMoney += hourChange * 0.9; // Keep more profit for re-investment
            log.w(`New spending account amount (HOURLY UPDATE) - ${spendingMoney}`, "Market: ");
        } else {
            spendingMoney += hourChange; // If no profit, just adjust with the change
            log.w(`New spending account amount (HOURLY UPDATE) - ${spendingMoney}`, "Market: ");
        }

        // Track resources for profitable buys
        if (hourChange > 0) {
            optimizeMarketBuying(hourChange); // Call function to make profitable purchases
        }
    ...
    // If a random influx is due
        ...
        if (Game.market.credits > CREDIT_BUFFER && Math.random() > 0.5 && spendingMoney < 1000) {
            ...
            spendingMoney += bankersCut * 0.1; // Allocate 10% for more flexible spending
    ...
    // Optimize buying strategy based on market prices and fluctuations
    function optimizeMarketBuying(profit) {
        ...
        globalOrders.forEach(order => {
            if (order.type === ORDER_BUY && order.price <= latestMarketHistory(order.resourceType).avg * buyingThreshold) {
                ...
                if (availableAmount * order.price <= profit * 0.25) {
                    let buyAmount = Math.min(availableAmount, Math.floor(profit / order.price));
                    log.w(`Buying ${buyAmount} ${order.resourceType} for ${order.price} credits each.`, "Market: ");
                    if (Game.market.deal(order.id, buyAmount) === OK) {
                        profit -= buyAmount * order.price;  // Deduct the purchase from the profit
                        spendingMoney -= buyAmount * order.price;  // Deduct the purchase from the spending account
                        log.w(`Bought ${buyAmount} ${order.resourceType} for ${buyAmount * order.price} credits. New spending account amount - ${spendingMoney}`, "Market: ");
                    }
                }
            }
        });
...
function tradeDiplomacyTracker() {
    ...
    let incoming = Game.market.incomingTransactions.filter(t => t && t.time > lastCheckedIncoming);
    ...
    if (incoming.length > 0) {
        ...
        for (let trade of incoming) {
            ...
            if (trade.sender && trade.sender.username) {
                ...
                processUserStanding(trade.sender.username, increase);
            }
        }
        // Update the lastCheckedIncoming timestamp
        lastCheckedIncoming = Math.max(...incoming.map(t => t.time));
    }
...
// Helper function to update a user's standing
function processUserStanding(username, increase) {
    if (!Memory._userList) {
