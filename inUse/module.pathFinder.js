...
function Move(creep, target, maxRooms = 1) {
    ...
    if (creep.memory.pathPos === JSON.stringify(creep.pos)) {
        creep.memory.pathPosTime++;
    } else {
        creep.memory.pathPos = JSON.stringify(creep.pos);
        creep.memory.pathPosTime = 1;
    }
    if (creep.memory.pathPosTime > 2) {
    ...
    if (creep.memory.pathAge >= creep.memory.pathLimit) {
        if (cache.getPath(creep.pos, target.pos)) {
            ...
            creep.memory.path = creep.room.findPath(creep.pos, target.pos, {
                costCallback: function (roomName, costMatrix) {
                    ...
                    if (hostileCreeps.length > 0) {
                        ...
                        for (let i = 0; i < hostileCreeps.length; i++) {
                            ...
                            costMatrix.set(hostileCreeps[i].pos.x+1, hostileCreeps[i].pos.y+1, 255);
                            costMatrix.set(hostileCreeps[i].pos.x, hostileCreeps[i].pos.y+1, 255);
                            costMatrix.set(hostileCreeps[i].pos.x+1, hostileCreeps[i].pos.y, 255);
                            costMatrix.set(hostileCreeps[i].pos.x+1, hostileCreeps[i].pos.y-1, 255);
                            costMatrix.set(hostileCreeps[i].pos.x-1, hostileCreeps[i].pos.y+1, 255);
                            costMatrix.set(hostileCreeps[i].pos.x-1, hostileCreeps[i].pos.y-1, 255);
                            costMatrix.set(hostileCreeps[i].pos.x, hostileCreeps[i].pos.y-1, 255);
                ...
                maxOps: 20000, serialize: true, ignoreCreeps: cacheWorthy, maxRooms: maxRooms, plainCost: 5, swampCost: 15
            });
            if (cacheWorthy === true) {
...
function MoveToPos(creep, target, maxRooms = 1) {
    creep.memory.pathAge++;
    if (creep.fatigue > 0) {
        creep.room.visual.circle(creep.pos, {fill: 'transparent', radius: 0.55, stroke: 'black'});
        return;
    ...
    if (creep.moveByPath(creep.memory.path) !== OK) {
        ...
        creep.memory.path = creep.room.findPath(creep.pos, target, {
            ...
            maxOps: 20000, serialize: true, ignoreCreeps: true, maxRooms: maxRooms, plainCost: 5, swampCost: 15
        });
        if (cacheWorthy === true) {
            cache.cachePath(creep.pos, target, creep.memory.path);
        }
