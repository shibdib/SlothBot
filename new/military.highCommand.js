...
function highCommand() {
    for (let key in Memory.ownedRooms) {
        ...
        let localTargets = _.filter(Memory.roomCache, (r) => r.cached > Game.time - 1000 && r.owner && !_.includes(RawMemory.segments[2], r.owner['username']) && Game.map.findRoute(r.name, Memory.ownedRooms[key].name).length <= 3);
