...
Creep.prototype.borderPatrol = function () {
    ...
    if (this.memory.squadLeader) {
        ...
        let squadLeader = _.filter(Game.creeps, (c) => c.memory && c.memory.operation === this.memory.operation && c.memory.squadLeader && c.id !== this.id
            && c.memory.targetRoom === this.memory.targetRoom);
        ...
        if (this.handleMilitaryCreep(false, true, true, false, true)) {
            return this.memory.contactReport = true;
        } else if (this.room.name !== this.memory.responseTarget) {
            this.attackInRange();
        }
