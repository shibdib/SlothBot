...
function shibMove(creep, heading, options = {}) {
    ...
    if (pathInfo.pathPosTime && pathInfo.pathPosTime >= STATE_STUCK) {
        let bumpCreep = _.filter(creep.room.creeps, (c) => c.memory && !c.memory.trailer && c.getActiveBodyparts(MOVE) && c.pos.x === pathInfo.newPos.x && c.pos.y === pathInfo.newPos.y && (!c.memory._shibMove || !c.memory._shibMove.path))[0];
        if (bumpCreep) {
