...
abandon = function (room) {
  if (!Game.rooms[room] || !Game.rooms[room].memory.bunkerHub)
    return log.e(room + " does not appear to be owned by you.");
  let overlordFor = _.filter(
    Game.creeps,
    (c) => c.memory && c.memory.overlord === room
  );
  ...
  if (target)
    nukes = _.filter(
      ...
      (s) =>
        ...
        !s.store.getFreeCapacity(RESOURCE_GHODIUM) &&
        !s.cooldown &&
        Game.map.getRoomLinearDistance(s.room.name, target) <= 10
    );
  if (!nukes.length && !target) return log.a("No nukes available");
  if (!nukes.length && target)
    return log.a("No nukes available in range of " + target);
  for (let key in nukes) {
    if (target)
      log.a(
        nukes[key].room.name +
          " has a nuclear missile available that is in range of " +
          target
      );
    if (!target)
      log.a(nukes[key].room.name + " has a nuclear missile available.");
  }
};
...
status = function () {
  ...
  log.e(
    "GCL - " +
      Game.gcl.level +
      " | GCL Progress - " +
      _.round(Game.gcl.progress / Game.gcl.progressTotal, 2) * 100 +
      "% | Creep Count - " +
      _.size(Game.creeps),
    " "
  );
  try {
    ...
    for (let activeRoom of myRooms) {
      ...
      let marauder,
        averageCpu = "No Data";
      if (ROOM_CPU_ARRAY[activeRoom.name])
        averageCpu =
          _.round(average(ROOM_CPU_ARRAY[activeRoom.name]), 2) || "No Data";
      let roomCreeps = _.filter(
        Game.creeps,
        (c) => c.memory && c.memory.overlord === activeRoom.name
      );
      let marauderText = "";
      let marauderCreep = _.filter(
        roomCreeps,
        (c) => c.memory.operation === "marauding"
      )[0];
      if (marauderCreep) {
      ...
      log.e(
        roomLink(activeRoom.name) +
          lowPowerText +
          " | RCL - " +
          activeRoom.controller.level +
          " | CPU Usage - " +
          averageCpu +
          " | RCL Progress - " +
          _.round(
            activeRoom.controller.progress /
              activeRoom.controller.progressTotal,
            2
          ) *
            100 +
          "% | Energy Available - " +
          ...
          " | Avg. Energy Income - " +
          _.round(
            average(JSON.parse(ROOM_ENERGY_INCOME_ARRAY[activeRoom.name])),
            0
          ) +
          " " +
          marauderText +
  ...
  try {
    ...
    if (operations && _.size(operations)) {
      ...
      for (let key in operations) {
        ...
        if (operations[key].priority === 3) priority = "Increased";
        else if (operations[key].priority === 2) priority = "High";
        else if (operations[key].priority === 1) priority = "Urgent";
        if (operations[key].enemyDead || operations[key].friendlyDead) {
          log.e(
            _.capitalize(type) +
              ...
              roomLink(key) +
              " | Enemy KIA - " +
              operations[key].trackedEnemy.length +
              "/" +
              operations[key].enemyDead +
              " | Friendly KIA - " +
              operations[key].trackedFriendly.length +
              "/" +
              operations[key].friendlyDead,
            " "
          );
  ...
  try {
    if (Memory._badBoyArray && Memory._badBoyArray.length) {
      ...
      if (Memory._enemies && Memory._enemies.length)
        log.e("Current Enemies: " + Memory._enemies.join(", "), " ");
      if (Memory._nuisance && Memory._nuisance.length)
        log.e("Current Nuisances: " + Memory._nuisance.join(", "), " ");
    }
  } catch (e) {
