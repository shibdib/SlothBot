...
function rcl2Extensions(spawn) {
    ...
    const pos4 = new RoomPosition(spawn.pos.x+2, spawn.pos.y-1, spawn.room.name);
    ...
    const pos5 = new RoomPosition(spawn.pos.x-3, spawn.pos.y+1, spawn.room.name);
...
function rcl3Extensions(spawn) {
    ...
    const pos4 = new RoomPosition(spawn.pos.x-3, spawn.pos.y-2, spawn.room.name);
    if (pos4.lookFor(LOOK_STRUCTURES).length === 0 && pos4.lookFor(LOOK_CONSTRUCTION_SITES).length === 0) {
        pos4.createConstructionSite(STRUCTURE_EXTENSION);
    }
