...
Object.defineProperty(PowerCreep.prototype, "idle", {
  ...
  get: function () {
    ...
    if (
      this.pos.checkForRoad() &&
      this.memory.role !== "stationaryHarvester" &&
      this.memory.role !== "upgrader" &&
      this.memory.role !== "mineralHarvester" &&
      this.memory.role !== "remoteHarvester"
    ) {
      this.moveRandom();
    } else if (
      this.pos.getRangeTo(this.pos.findClosestByRange(FIND_MY_SPAWNS)) === 1
    ) {
      this.moveRandom();
    } else {
...
PowerCreep.prototype.reportDamage = function () {
  ...
  if (this.memory.healsInbound && !Game.getObjectById(this.memory.healsInbound))
    this.memory.healsInbound = undefined;
  ...
  if (this.hits < this.memory._lastHits) {
    ...
    if (
      ...
      ((this.room.controller.owner &&
        this.room.controller.owner.username !== MY_USERNAME) ||
        (this.room.controller.reservation &&
          this.room.controller.reservation.username !== MY_USERNAME)) &&
      this.memory.destination !== this.room.name
    )
      return false;
    let nearbyCreeps = _.uniq(
      _.pluck(
        _.filter(
          this.room.creeps,
          (c) =>
            c.pos.getRangeTo(this) <= 3 &&
...
PowerCreep.prototype.fleeNukeRoom = function () {
  ...
  if (this.memory.fleeTo && this.room.name !== this.memory.fleeTo)
    this.shibMove(new RoomPosition(25, 25, this.memory.fleeTo), { range: 23 });
  else if (this.room.name !== this.memory.fleeTo)
    this.idleFor(this.memory.fleeNukeTime - Game.time);
  if (!this.memory.fleeTo)
    this.memory.fleeTo = _.sample(
      _.filter(Memory.myRooms, (r) => !r.nukes.length)
    ).name;
};
