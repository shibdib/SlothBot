...
function labTech(creep) {
    ...
    for (let key in labs) {
        if (Memory.structures && Memory.structures[labs[key].id] && Memory.structures[labs[key].id].itemNeeded && (labs[key].mineralType !== Memory.structures[labs[key].id].itemNeeded || (labs[key].mineralType === Memory.structures[labs[key].id].itemNeeded && labs[key].mineralAmount < 250))) {
