...
module.exports.linkControl = function (room) {
  let links = shuffle(
    _.filter(
      ...
      (s) =>
        ...
        s.id !== s.room.memory.hubLink
    )
  );
  let hubLink = Game.getObjectById(room.memory.hubLink);
  let controllerLink = Game.getObjectById(room.memory.controllerLink);
  if (Math.random() > 0.8) {
    if (!controllerLink || !controllerLink.isActive())
  ...
  for (let link of links) {
    if (!controllerLink) {
      if (
        _.filter(
          link.pos.findInRange(room.structures, 2),
    ...
    if (
      upgrader &&
      ((controllerLink && controllerLink.energy < 50 && Math.random() > 0.5) ||
        (room.energyState && controllerLink && controllerLink.energy < 450))
    ) {
      ...
      _.filter(
        ...
        (l) =>
          l.id !== link.id &&
          l.energy < l.energyCapacity * 0.5 &&
          l.energy < link.energy
      )[0]
    ) {
      link.transferEnergy(
        _.filter(
          links,
          (l) => l.id !== link.id && l.energy < l.energyCapacity * 0.5
        )[0],
        link.energy * 0.5
      );
    }
  ...
  if (
    ...
    controllerLink &&
    hubLink.energy > 100 &&
    controllerLink.energy < 250 &&
    hubLink.room.energyAvailable > hubLink.room.energyCapacityAvailable * 0.95
  ) {
    hubLink.transferEnergy(controllerLink);
  } else if (
    hubLink &&
    controllerLink &&
    controllerLink.energy > 400 &&
    hubLink.energy < 50 &&
    hubLink.room.energyAvailable < hubLink.room.energyCapacityAvailable * 0.95
  ) {
