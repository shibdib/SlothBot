...
module.exports.processBuildQueue = function () {
  ...
  for (let key in spawns) {
    ...
    if (
      lastBuilt[spawn.room.name] &&
      roomQueue[spawn.room.name] &&
      lastBuilt[spawn.room.name] + 2000 < Game.time
    ) {
    ...
    if (level > spawn.room.controller.level)
    ...
    if (!spawn.spawning) {
      if (roomQueue[spawn.room.name] || globalQueue) {
        ...
        if (
          !spawn.room.memory.nuke &&
          _.size(globalQueue) &&
          !Memory.roomCache[spawn.room.name].responseNeeded &&
          _.inRange(level, maxLevel - 1, maxLevel + 1) &&
          !spawn.room.memory.lowPower
        ) {
          // Filter ops by range
          let range = LOCAL_SPHERE * 2;
          if (spawn.room.energyState) range = LOCAL_SPHERE * 4;
          let distanceFilteredGlobal = _.filter(
            globalQueue,
            (q) =>
              q.destination &&
              (Memory.auxiliaryTargets[q.destination] ||
                Game.map.getRoomLinearDistance(q.destination, spawn.room.name) <
                  range)
          );
          queue = _.sortBy(
            Object.assign(
              {},
              distanceFilteredGlobal,
              roomQueue[spawn.room.name]
            ),
            "priority"
          );
        ...
        for (let key in queue) {
          ...
          if (
            role === "defender" &&
            !Memory.roomCache[spawn.room.name].threatLevel
          ) {
            if (roomQueue[spawn.room.name])
              delete roomQueue[spawn.room.name][role];
            continue;
          }
          if (topPriority.misc && topPriority.misc === "vary")
            level = _.random(_.round(level / 1.5), level);
          ...
          if (
            topPriority.destination &&
            (Game.map.findRoute(topPriority.destination, spawn.room.name)
              .length > 20 ||
              (_.includes(body, CLAIM) &&
                Game.map.findRoute(topPriority.destination, spawn.room.name)
                  .length > 7))
          )
            continue;
        ...
        if (topPriority && typeof topPriority === "object") {
          ...
          if (topPriority.operation)
            name = _.uniqueId(
              topPriority.operation + "_" + spawn.room.name + "_T" + level + "_"
            );
          let energyStructures;
          if (energyOrder[spawn.pos.roomName])
            energyStructures = JSON.parse(energyOrder[spawn.pos.roomName]);
          let success;
          while (!success) {
            let name = _.uniqueId(
              role + "_" + spawn.room.name + "_T" + level + "_"
            );
            switch (
              ...
              case OK:
                ...
                success = true;
                if (!topPriority.operation)
                  log.d(spawn.room.name + " Spawning a " + role);
                if (topPriority.military && globalQueue)
                  delete globalQueue[role];
                if (topPriority.buildCount && roomQueue[spawn.room.name][role])
                  return (roomQueue[spawn.room.name][role].buildCount =
                    topPriority.buildCount - 1);
                if (roomQueue[spawn.room.name])
                  delete roomQueue[spawn.room.name][role];
                break;
      ...
      spawn.say(
        ICONS.build +
          " " +
          _.capitalize(spawningCreep.name.split("_")[0]) +
          " - Ticks: " +
...
module.exports.roomStartup = function (room) {
    ...
    let roomCreeps = _.filter(
        Game.creeps,
        (r) => r.memory.overlord === room.name
    );
    ...
    let harvesters = _.filter(
        roomCreeps,
        (c) => c.memory.role === "stationaryHarvester"
    );
    if (!_.includes(queue, "stationaryHarvester")) {
        if (
            harvesters.length < 2 ||
            (harvesters[0].ticksToLive < harvesters[0].body.length * 3 + 10 &&
                harvesters.length < 3)
        ) {
    ...
    if (!_.includes(queue, "hauler")) {
        if (
            hauler.length < 2 ||
            (hauler[0].ticksToLive < hauler[0].body.length * 3 + 10 &&
                hauler.length < 3)
        ) {
    ...
    if (
        !_.includes(queue, "explorer") &&
        !Memory.roomCache[room.name].responseNeeded
    ) {
        let amount = 6;
        let explorers = _.filter(
            roomCreeps,
...
module.exports.essentialCreepQueue = function (room) {
  ...
  if (harvesters.length === 0) {
    ...
    let harvesters = _.filter(
      roomCreeps,
      (c) => c.memory.role === "stationaryHarvester" && !c.memory.other.reboot
    );
    if (
      harvesters.length < 2 ||
      (harvesters[0].ticksToLive < harvesters[0].body.length * 3 + 10 &&
  ...
  if (room.memory.hubLink || room.storage) {
    ...
    if (!hauler.length) {
      ...
      if (
        (hauler[0] &&
          hauler[0].ticksToLive < hauler[0].body.length * 6 + 50 &&
          hauler.length < amount + 1) ||
        hauler.length < amount
      ) {
  ...
  let noLink = _.filter(
    harvesters,
    (c) => !c.memory.linkID && c.memory.linkAttempt
  )[0];
  if (noLink) {
    let amount =
      2 +
      _.filter(
        ...
        (s) =>
          ...
          s.store[RESOURCE_ENERGY] >= 1800
      ).length;
    let filler = _.filter(roomCreeps, (c) => c.memory.role === "filler");
    if (
      (filler[0] &&
        filler[0].ticksToLive < filler[0].body.length * 3 + 10 &&
        filler.length < amount + 1) ||
      filler.length < amount
    ) {
      if (filler.length === 0) {
        return queueCreep(room, -1, {
          role: "filler",
  ...
  if (Memory.roomCache[room.name].threatLevel >= 2) {
    let responder = _.filter(
      Game.creeps,
      (creep) =>
        creep.my &&
        creep.memory.other &&
        creep.memory.other.responseTarget === room.name
    );
  ...
  upgraders.forEach(
    (h) =>
      (upgradePower += h.getActiveBodyparts(WORK) * UPGRADE_CONTROLLER_POWER)
  );
  if (
    level < 8 &&
    !inBuild &&
    upgradePower <
      ROOM_ENERGY_PER_TICK[room.name] * ROOM_ENERGY_ALLOTMENT["upgrade"] &&
    upgraders.length < 8 - room.level
  )
    number = upgraders.length + 1;
  if (
    room.storage &&
    (!ROOM_ENERGY_INCOME_ARRAY[room.name] ||
      average(JSON.parse(ROOM_ENERGY_INCOME_ARRAY[room.name])) <= 5)
  )
    number *= deficit;
  if (level >= 5 && number > 2) number = 2;
  if (level < 3) number += 3;
  if (
    upgraders.length < number ||
    (upgraders[0] &&
      upgraders[0].ticksToLive < upgraders[0].body.length * 3 + 10 &&
      upgraders.length < number + 1)
  ) {
    //If room is about to downgrade get a creep out asap
      let reboot =
          room.controller.ticksToDowngrade <= CONTROLLER_DOWNGRADE[level] * 0.9 ||
          room.controller.progress > room.controller.progressTotal ||
          Memory.roomCache[room.name].threatLevel >= 3 ||
          room.memory.lowPower;
      queueCreep(room, PRIORITIES.upgrader + upgraders.length * 0.2, {
          role: "upgrader",
          other: {reboot: reboot},
      });
  }
...
module.exports.praiseCreepQueue = function (room) {
  ...
  if (inBuild || room.controller.level === 1) {
    ...
    if (drones.length < amount) {
      queueCreep(room, 1 + drones.length, {
        role: "drone",
  ...
  if (harvesters.length) {
    ...
    if (
      (filler[0] &&
        filler[0].ticksToLive < filler[0].body.length * 3 + 10 &&
        filler.length < harvesters.length + 1) ||
      filler.length < harvesters.length
    ) {
      queueCreep(room, PRIORITIES.hauler - 1, {
        role: "filler",
        other: {
  ...
  if (Memory.roomCache[room.name].threatLevel) {
    let responder = _.filter(
      Game.creeps,
      (creep) => creep.memory.other.responseTarget === room.name
    );
  ...
  if (level >= 2) {
    let waller = _.filter(
      roomCreeps,
      (creep) => creep.memory.role === "waller"
    );
    let amount = 1;
  ...
  //Mineral Harvester
  let extractor = room.structures.filter(
    (s) => s.structureType === STRUCTURE_EXTRACTOR
  )[0];
  if (extractor && room.mineral.mineralAmount) {
    let praiseMineral = _.filter(
      Game.creeps,
      (creep) =>
        creep.memory.overlord === room.name &&
        creep.memory.role === "praiseMineral"
    );
  ...
  let upgraders = _.filter(
    roomCreeps,
    (creep) => creep.memory.role === "praiseUpgrader"
  );
  let upgradePower = 0;
  let number = 1;
  upgraders.forEach(
  ...
  if (upgradePower < ROOM_ENERGY_PER_TICK[room.name] * 0.75)
    number = upgraders.length + 1;
  if (
    upgraders.length < number ||
    (upgraders[0] &&
    ...
    queueCreep(room, PRIORITIES.upgrader + upgraders.length, {
      role: "praiseUpgrader",
    });
  }
  // Food
  if (!_.size(roomQueue[room.name])) {
    let needFood =
      _.filter(
        roomCreeps,
        (creep) =>
          creep.memory.role === "praiseUpgrader" &&
          _.sum(creep.store) < creep.store.getCapacity() * 0.15
      ).length > 0 &&
      !_.filter(roomCreeps, (creep) => creep.memory.role === "food").length;
    if (needFood) {
      let praisePower = 0;
      upgraders.forEach(
        (h) =>
          (praisePower += h.getActiveBodyparts(WORK) * UPGRADE_CONTROLLER_POWER)
      );
      praisePower *= 4;
      let body;
      if (praisePower >= 175) body = [HEAL];
      else if (praisePower >= 110) body = [RANGED_ATTACK];
      else if (praisePower >= 80) body = [WORK];
      else if (praisePower >= 50) body = [ATTACK];
      else body = [CARRY];
      let spawn = _.filter(
        room.structures,
...
module.exports.miscCreepQueue = function (room) {
  ...
  let roomCreeps = _.filter(
    Game.creeps,
    (r) =>
      r.memory.overlord === room.name &&
      (!r.memory.destination || r.memory.destination === room.name)
  );
  //Drones
  let inBuild = _.filter(
    room.constructionSites,
  ...
  if (inBuild) {
    ...
    let drones = _.filter(
      roomCreeps,
      (c) => c.memory.role === "drone" && !c.memory.destination
    );
    drones.forEach(
      (h) => (buildPower += h.getActiveBodyparts(WORK) * BUILD_POWER)
    );
    if (
      buildPower <
      ROOM_ENERGY_PER_TICK[room.name] * ROOM_ENERGY_ALLOTMENT["build"]
    )
      number = drones.length + 1;
    if (!nonRoads) number = 1;
    if (drones.length < number) {
      queueCreep(room, PRIORITIES.drone + drones.length * 0.2, {
        role: "drone",
        other: { localCache: false },
      });
    }
  ...
  if (!room.nukes.length && room.terminal) {
    let labTech = _.filter(
      roomCreeps,
      (creep) => creep.memory.role === "labTech"
    );
    if (!labTech.length) {
      queueCreep(room, PRIORITIES.miscHauler, {
        role: "labTech",
        other: { localCache: false },
      });
    }
  ...
  if (
    ...
    room.energyState &&
    level === 8 &&
    room.store(RESOURCE_POWER) &&
    _.filter(
      room.structures,
      (s) => s.structureType === STRUCTURE_POWER_SPAWN
    )[0]
  ) {
    let powerManager = _.filter(
      roomCreeps,
      (creep) => creep.memory.role === "powerManager"
    );
  ...
  if (!room.nukes.length && level >= 6 && room.mineral.mineralAmount) {
    let mineralHarvester = _.filter(
      Game.creeps,
      (creep) =>
        creep.memory.overlord === room.name &&
        creep.memory.role === "mineralHarvester"
    );
    let extractor = room.structures.filter(
      (s) => s.structureType === STRUCTURE_EXTRACTOR
    )[0];
  ...
  if (!room.nukes.length && !Memory.roomCache[room.name].threatLevel) {
    ...
    let totalExplorers = _.filter(
      Game.creeps,
      (creep) => creep.memory.role === "explorer"
    );
    let roomExplorers = _.filter(
      Game.creeps,
      (creep) =>
        creep.memory.overlord === room.name && creep.memory.role === "explorer"
    );
    if (
      level < 8 &&
      (totalExplorers.length < amount || !roomExplorers.length)
    ) {
    ...
    // Assist room
    if (level >= 3) {
      let safeToSupport = _.filter(
        Memory.myRooms,
        (r) =>
          !Memory.roomCache[r] ||
          !Memory.roomCache[r].lastPlayerSighting ||
          Memory.roomCache[r].lastPlayerSighting + 100 < Game.time
      );
      let needDrones = _.sample(
        _.filter(
          safeToSupport,
          (r) =>
            r !== room.name &&
            Game.map.getRoomLinearDistance(r, room.name) < 10 &&
            Game.rooms[r].memory.buildersNeeded
        )
      );
      if (needDrones) {
        let drones = _.filter(
          Game.creeps,
          (creep) =>
            (creep.memory.destination === needDrones ||
              creep.memory.overlord === needDrones) &&
            creep.memory.role === "drone"
        );
        let amount = ROOM_SOURCE_SPACE[needDrones] || 2;
        if (
          drones.length < amount ||
          (drones[0].ticksToLive <
            drones[0].body.length * 3 +
              10 * Game.map.getRoomLinearDistance(needDrones, room.name) &&
            drones.length < amount + 1)
        ) {
      ...
      if (level >= 6 && room.energyState) {
        // Energy Supplies
        let needEnergy = _.sample(
          _.filter(
            safeToSupport,
            (r) =>
              r !== room.name &&
              !Game.rooms[r].energyState &&
              !Game.rooms[r].memory.praiseRoom &&
              Game.map.getRoomLinearDistance(r, room.name) < 6 &&
              !Game.rooms[r].terminal
          )
        );
        if (needEnergy) {
          let fuelTruck = _.filter(
            Game.creeps,
            (creep) =>
              creep.memory.destination === needEnergy &&
              creep.memory.role === "fuelTruck"
          );
          if (!fuelTruck.length) {
        ...
        // Power Level
        let upgraderRequested = _.sample(
          _.filter(
            safeToSupport,
            (r) =>
              r !== room.name &&
              !Game.rooms[r].memory.praiseRoom &&
              Game.rooms[r].controller.level + 1 < level &&
              Game.map.getRoomLinearDistance(r, room.name) < 6
          )
        );
        if (upgraderRequested) {
          let remoteUpgraders = _.filter(
            Game.creeps,
            (creep) =>
              creep.memory.destination === upgraderRequested &&
              creep.memory.role === "remoteUpgrader"
          );
          if (!remoteUpgraders.length) {
            queueCreep(
              room,
              PRIORITIES.remoteUpgrader + remoteUpgraders.length,
              {
                role: "remoteUpgrader",
      ...
      // Marauder
      if (
        (Memory._badBoyArray && Memory._badBoyArray.length) ||
        NEW_SPAWN_DENIAL
      ) {
        let poke = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.overlord === room.name && creep.memory.role === "poke"
        );
        if (!poke.length) {
  ...
  if (room.memory.spawnBorderPatrol) {
    ...
    let count = 2;
    if (
      borderPatrol.length < count ||
      (borderPatrol[0] &&
        borderPatrol[0].ticksToLive < borderPatrol[0].body.length * 3 + 10 &&
        borderPatrol.length < count + 1)
    ) {
...
module.exports.remoteCreepQueue = function (room) {
  ...
  if (!remoteHives[room.name] || Math.random() > 0.95) {
    ...
    let adjacent = _.filter(
      Game.map.describeExits(room.name),
      (r) =>
        Memory.roomCache[r] &&
        !Memory.roomCache[r].isHighway &&
        Game.map.getRoomStatus(r).status ===
          Game.map.getRoomStatus(room.name).status &&
        (!Memory.roomCache[r].user ||
          Memory.roomCache[r].user === MY_USERNAME) &&
        !Memory.roomCache[r].owner
    );
    ...
    if (!adjacent.length) {
      ...
      if (highway.length) {
        adjacent = _.filter(
          Game.map.describeExits(highway[0]),
          (r) =>
            Memory.roomCache[r] &&
            !Memory.roomCache[r].isHighway &&
            (!Memory.roomCache[r].user ||
    ...
    if (adjacent.length < 5 - room.energyState) {
      ...
      for (let adjacentRoom of adjacent) {
        let secondaryAdjacent = _.filter(
          Game.map.describeExits(adjacentRoom),
          (r) =>
            Memory.roomCache[r] &&
            Memory.roomCache[r].sources &&
            (!Memory.roomCache[r].user ||
              Memory.roomCache[r].user === MY_USERNAME) &&
            Game.map.getRoomStatus(r).status ===
              Game.map.getRoomStatus(room.name).status &&
            !Memory.roomCache[r].owner &&
            !Memory.roomCache[r].sk &&
            Game.map.getRoomLinearDistance(room.name, r) <= 1
        );
        if (secondaryAdjacent.length)
          secondary = secondary.concat(secondaryAdjacent);
      }
  ...
  //Remotes
  if (
    !room.memory.turtleMode &&
    remoteHives[room.name] &&
    !Memory.roomCache[room.name].responseNeeded
  ) {
    room.memory.spawnBorderPatrol = undefined;
    ...
    for (let keys in shuffle(remotes)) {
      ...
      if (Memory.avoidRemotes && _.includes(Memory.avoidRemotes, remoteName))
        continue;
      // Handle invaders
      if (
        Memory.roomCache[remoteName] &&
        (Memory.roomCache[remoteName].invaderCore ||
          Memory.roomCache[remoteName].threatLevel ||
          Memory.roomCache[remoteName].responseNeeded) &&
        !Memory.roomCache[remoteName].towers
      ) {
        if (
          Memory.roomCache[remoteName].invaderTTL &&
          Memory.roomCache[remoteName].invaderTTL < Game.time
        ) {
          let scout = _.filter(
            Game.creeps,
            (creep) =>
              creep.memory.destination === remoteName &&
      ...
      if (
        Memory.roomCache[remoteName] &&
        (Memory.roomCache[remoteName].level ||
          !Memory.roomCache[remoteName].sources)
      )
        continue;
      ...
      if (
        Memory.roomCache[remoteName] &&
        Memory.roomCache[remoteName].reservation === "Invader"
      )
        invaderCoreReserved = true;
      if (
        !invaderCoreReserved &&
        Memory.roomCache[remoteName] &&
        Memory.roomCache[remoteName].reservation &&
        Memory.roomCache[remoteName].reservation !== MY_USERNAME
      )
        continue;
      // Handle SK
      if (
        Memory.roomCache[remoteName] &&
        Memory.roomCache[remoteName].sk &&
        level >= 7 &&
        !Memory.roomCache[remoteName].invaderCore &&
        !room.memory.lowPower
      ) {
        skMining = true;
        let SKAttacker = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.destination === remoteName &&
            creep.memory.role === "SKAttacker"
        );
        if (
          (SKAttacker[0] &&
            SKAttacker[0].ticksToLive < SKAttacker[0].body.length * 3 + 150 &&
            SKAttacker.length < 2) ||
          SKAttacker.length < 1
        ) {
        ...
        if (
          room.energyState !== 2 &&
          SKHarvester.length < sourceCount &&
          SKAttacker.length
        ) {
        ...
        let SKMineral = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.destination === remoteName &&
            creep.memory.role === "SKMineral"
        );
        if (
          !SKMineral.length &&
          SKAttacker.length &&
          (!Memory.roomCache[remoteName].mineralCooldown ||
            Memory.roomCache[remoteName].mineralCooldown < Game.time)
        ) {
        ...
        if (!invaderCoreReserved && !room.memory.lowPower) {
          let totalHarvester = _.filter(
            Game.creeps,
            (creep) =>
              creep.memory.overlord === room.name &&
              creep.memory.role === "remoteHarvester"
          ).length;
          if (totalHarvester < 11 - room.level) {
            let remoteHarvester = _.filter(
              Game.creeps,
              (creep) =>
                creep.memory.destination === remoteName &&
                creep.memory.role === "remoteHarvester"
            ...
            if (
              ...
              sourceCount = Memory.roomCache[remoteName].sources;
            if (
              remoteHarvester.length < sourceCount ||
              (remoteHarvester[0] &&
                remoteHarvester[0].ticksToLive <
                  remoteHarvester[0].body.length * 3 + 10 &&
                remoteHarvester.length < sourceCount + 1)
            ) {
              room.memory.lastRemoteAttempt = Game.time;
              queueCreep(
                room,
                PRIORITIES.remoteHarvester + remoteHarvester.length,
                {
                  role: "remoteHarvester",
                  destination: remoteName,
                }
        ...
        if (
          level >= 4 &&
          Memory.roomCache[remoteName] &&
          (!Memory.roomCache[remoteName].reservationExpires ||
            Game.time > Memory.roomCache[remoteName].reservationExpires) &&
          Memory.roomCache[remoteName].sources < 3
        ) {
          let reserver = _.filter(
            Game.creeps,
            (creep) =>
              creep.memory.role === "reserver" &&
              creep.memory.other.reservationTarget === remoteName
          );
          let amount = 1;
          if (
            Memory.roomCache[remoteName] &&
            Memory.roomCache[remoteName].reserverCap
          )
            amount = Memory.roomCache[remoteName].reserverCap;
    ...
    let remoteHarvester = _.filter(
      ...
      (creep) =>
        creep.memory.role === "remoteHarvester" &&
        _.includes(JSON.parse(remoteHives[room.name]), creep.room.name)
    );
    if (remoteHarvester.length && !room.memory.lowPower) {
      let remoteHaulers = _.filter(
        Game.creeps,
        (creep) =>
          creep.my &&
      ...
      if (remoteHaulers.length < remoteHarvester.length * 0.5) {
...
module.exports.globalCreepQueue = function () {
  ...
  //Marauder
  let marauder = _.filter(
    Game.creeps,
    (creep) =>
      creep.memory.operation === "marauding" && creep.memory.role === "longbow"
  );
  ...
  for (let key in shuffle(operations)) {
    ...
    if (opLevel === 0 && !operations[key].observerCheck) {
      let scout = _.filter(
        Game.creeps,
        (creep) =>
          creep.memory.destination === key && creep.memory.role === "scout"
      );
      if (
        scout.length < 1 ||
        (scout[0] &&
          scout[0].ticksToLive < scout[0].body.length * 3 + 10 &&
          scout.length < 2)
      ) {
        queueGlobalCreep(PRIORITIES.priority, {
          role: "scout",
    ...
    switch (operations[key].type) {
      ...
      case "claim":
        if (
          !_.filter(
            Game.creeps,
            (c) => c.memory.role === "claimer" && c.memory.destination === key
          ).length
        ) {
          queueGlobalCreep(PRIORITIES.claimer, {
            role: "claimer",
            destination: key,
            military: true,
          });
      ...
      case "commodity": //commodity Mining
        let commodityMiner = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.role === "commodityMiner" &&
            creep.memory.destination === key
        );
        let amount = Memory.roomCache[key].commodity || 2;
        if (_.isNaN(amount)) amount = 2;
        if (
          commodityMiner.length < amount ||
          (commodityMiner[0] &&
            commodityMiner[0].ticksToLive <
              commodityMiner[0].body.length * 3 + 100 &&
            commodityMiner.length < amount + 1)
        ) {
          queueGlobalCreep(PRIORITIES.Power, {
            role: "commodityMiner",
            destination: key,
            military: true,
          });
        }
        let commodityHauler = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.role === "commodityHauler" &&
            creep.memory.destination === key
        );
        if (commodityHauler.length < commodityMiner.length * 0.25) {
          queueGlobalCreep(PRIORITIES.Power - 1, {
            role: "commodityHauler",
            destination: key,
            military: true,
          });
        }
      ...
      case "power": //Power Mining
        ...
        let powerHealer = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.role === "powerHealer" &&
            creep.memory.destination === key
        );
        let powerAttacker = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.role === "powerAttacker" &&
            creep.memory.destination === key
        );
        if (
          !operations[key].complete &&
          !_.includes(queue, "powerHealer") &&
          (powerHealer.length < powerAttacker.length + 1 ||
            (powerHealer[0] &&
              powerHealer[0].ticksToLive <
                powerHealer[0].body.length * 3 + 250 &&
              powerHealer.length < powerAttacker.length + 1 + 1))
        ) {
          queueGlobalCreep(PRIORITIES.Power, {
            role: "powerHealer",
            destination: key,
        ...
        if (
          !operations[key].complete &&
          !_.includes(queue, "powerAttacker") &&
          (powerAttacker.length < powerSpace ||
            (powerAttacker[0] &&
              powerAttacker[0].ticksToLive <
                powerAttacker[0].body.length * 3 + 250 &&
              powerAttacker.length < powerSpace + 1))
        ) {
          queueGlobalCreep(PRIORITIES.Power - 1, {
            role: "powerAttacker",
            destination: key,
        ...
        if (
          operations[key].hauler &&
          !_.includes(queue, "powerHauler") &&
          powerHauler.length < operations[key].hauler
        ) {
          queueGlobalCreep(PRIORITIES.Power - 1, {
            role: "powerHauler",
            destination: key,
      ...
      case "hold": //Hold Room
        ...
        let longbow = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.destination === key &&
            creep.memory.role === "longbow" &&
            creep.memory.operation === "hold"
        );
        if (
          longbow.length < longbows ||
          (longbow[0] &&
            longbow[0].ticksToLive < longbow[0].body.length * 3 + 50 &&
            longbow.length < longbows + 1)
        ) {
        ...
        let claimAttacker = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.destination === key &&
            creep.memory.role === "claimAttacker" &&
            creep.memory.operation === "hold"
        );
        if (
          unClaimerNeeded &&
          (claimAttacker.length < 1 ||
            (claimAttacker[0] &&
              claimAttacker[0].ticksToLive <
                claimAttacker[0].body.length * 3 + 10 &&
              claimAttacker.length < 2)) &&
          longbow.length
        ) {
          queueGlobalCreep(priority, {
            role: "claimAttacker",
        ...
        let holdCleaner = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.destination === key &&
            creep.memory.role === "deconstructor"
        );
      ...
      case "siegeGroup": //Siege Group
        let siegeEngines = 1;
        let healers = 2;
        let siegeEngine = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.destination === key &&
            creep.memory.role === "siegeEngine" &&
            creep.memory.operation === "siegeGroup"
        );
        let healer = _.filter(
          Game.creeps,
          (creep) =>
        ...
        if (
          healer.length &&
          (siegeEngine.length < siegeEngines ||
            (siegeEngine[0] &&
              siegeEngine[0].ticksToLive <
                siegeEngine[0].body.length * 3 + 50 &&
              siegeEngine.length < siegeEngines + 1))
        ) {
        ...
        if (
          healer.length < healers ||
          (healer[0] &&
            healer[0].ticksToLive < healer[0].body.length * 3 + 50 &&
            healer.length < healers + 1)
        ) {
          queueGlobalCreep(priority, {
            role: "healer",
            destination: key,
      ...
      case "claimClear": //Claim Clearing
        let claimClear = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.destination === key && creep.memory.role === "claimer"
        );
      ...
      case "drain": // Drain
        ...
        let drainer = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.destination === key && creep.memory.role === "drainer"
        );
        if (
          drainer.length < drainers ||
          (drainer[0] &&
            drainer[0].ticksToLive < drainer[0].body.length * 3 + 50 &&
            drainer.length < drainers + 1)
        ) {
          queueGlobalCreep(priority, {
            role: "drainer",
            destination: key,
        ...
        break;
      case "siege": // Siege
        let siegeCreep = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.destination === key &&
            creep.memory.role === "siegeEngine"
        );
        let siegeHealer = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.destination === key &&
            creep.memory.role === "siegeHealer"
        );
      ...
      case "rangers": // Rangers
        ...
        let rangers = _.filter(
          ...
          (creep) =>
            creep.memory.destination === key &&
            creep.memory.role === "longbow" &&
            creep.memory.operation === "rangers"
        );
        if (
          rangers.length < number ||
          (rangers[0] &&
            rangers[0].ticksToLive < rangers[0].body.length * 3 + 10 &&
            rangers.length < number + 1)
        ) {
          queueGlobalCreep(priority, {
            role: "longbow",
            destination: key,
        ...
        if (
          (operations[key].claimAttacker && rangerUnClaimer.length < 1) ||
          (rangerUnClaimer[0] &&
            rangerUnClaimer[0].ticksToLive <
              rangerUnClaimer[0].body.length * 3 + 10 &&
            rangerUnClaimer.length < 2)
        ) {
          queueGlobalCreep(priority, {
            role: "claimAttacker",
            destination: key,
      ...
      case "guard": // Room Guard
        let guards = _.filter(
          Game.creeps,
          (creep) =>
            creep.memory.destination === key && creep.memory.role === "longbow"
        );
...
function queueCreep(room, importance, options = {}, military = false) {
  ...
  if (!military) {
    ...
    if (cache[options.role] && cache[options.role].priority <= importance)
      return;
  } else {
    cache = globalQueue || {};
    if (cache[options.role] && cache[options.role].priority <= importance)
...
function determineEnergyOrder(room) {
  ...
  if (!energyOrder[room.name] || orderStored[room.name] + 750 < Game.time) {
    let harvester = _.filter(
      room.creeps,
      (c) =>
        c.my && c.memory.role === "stationaryHarvester" && c.memory.onContainer
    );
    let hub = new RoomPosition(
      room.memory.bunkerHub.x,
      room.memory.bunkerHub.y,
      room.name
    );
    ...
    for (let x = 0; x < energyStructures.length; x++) {
      let nextClosest;
      let harvesterExtensions = _.filter(
        room.structures,
        (s) =>
          !_.includes(usedIdArray, s.id) &&
          (s.structureType === STRUCTURE_SPAWN ||
            s.structureType === STRUCTURE_EXTENSION) &&
          s.pos.findInRange(harvester, 1).length
      );
      if (harvesterExtensions.length) {
        nextClosest = harvesterExtensions[0];
      } else {
        nextClosest = hub.findClosestByPath(energyStructures, {
          filter: (s) => !_.includes(usedIdArray, s.id),
        });
      }
...
function displayQueue(room) {
  ...
  let distanceFilteredGlobal = _.filter(
    ...
    (q) =>
      q.destination &&
      Game.map.getRoomLinearDistance(q.destination, room.name) < range
  );
  ...
  if (
    ...
    queue = _.sortBy(
      Object.assign({}, distanceFilteredGlobal, roomQueue[room.name]),
      "priority"
    );
  } else {
    queue = _.sortBy(
      Object.assign(
        ...
        _.filter(
          distanceFilteredGlobal,
          (t) => t.priority <= PRIORITIES.urgent
        ),
        roomQueue[room.name]
      ),
      "priority"
  ...
  for (let i = 0; i < 5; i++) {
    ...
    displayText(
      ...
      35,
      2 + i,
      _.capitalize(roles[i]) +
        mil +
        " Priority- " +
        priority[i] +
        " Age- " +
        (Game.time - tickQueued[i])
    );
  }
}
