...
class RoleExplorer {
    ...
    findDestination() {
        ...
        if (!this.creep.memory.usedPortal && portal && (this.creep.memory.other.portalJump || Math.random() > 0.01)) {
            ...
            let adjacent = _.filter(_.map(Game.map.describeExits(this.room.name)), (r) => roomStatus(r) === roomStatus(this.creep.memory.overlord) && (!INTEL[this.room.name].obstacles || this.creep.pos.findPathTo(Game.map.findExit(this.room.name, r)).length));
