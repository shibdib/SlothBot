...
function operationPlanner(room, creep = undefined) {
  ...
  if (!Memory.targetRooms[room.name] && creep)
    return (creep.memory.role = "explorer");
  // Handle claim scout missions
  if (
    Memory.targetRooms[room.name] &&
    Memory.targetRooms[room.name].type === "claimScout"
  )
    return claimScout(room);
  // Handle forward observer
  if (
    Memory.targetRooms[room.name] &&
    Memory.targetRooms[room.name].type !== "attack" &&
    Memory.targetRooms[room.name].type !== "scout"
  )
  ...
  if (
    ...
    !Memory.roomCache[room.name].manual &&
    Memory.roomCache[room.name].lastOperation &&
    Memory.roomCache[room.name].lastOperation + ATTACK_COOLDOWN > Game.time
  ) {
  ...
  let towers = _.filter(
    room.structures,
    (s) =>
      s.structureType === STRUCTURE_TOWER && (s.isActive() || !room.controller)
  );
  ...
  if (controller && (controller.owner || controller.reservation)) {
    ...
    // Defend ally rooms
    if (controller.owner && _.includes(FRIENDLIES, controller.owner.username))
      ally = true;
    if (
      controller.reservation &&
      _.includes(FRIENDLIES, controller.reservation.username)
    )
      ally = true;
  }
  ...
  let priority = 4;
  if (range <= LOCAL_SPHERE) priority = 1;
  else if (range <= LOCAL_SPHERE * 1.25) priority = 2;
  else if (range <= LOCAL_SPHERE * 2) priority = 3;
  else priority = 4;
  // Plan op based on room comp
  let cache = Memory.targetRooms || {};
  let tick = Game.time;
  let otherCreeps = _.filter(
    room.creeps,
    (c) =>
      !c.my &&
      !_.includes(FRIENDLIES, c.owner.username) &&
  ...
  if (ally) {
    ...
    if (!controller) {
      ...
      if (
        !_.includes(Memory._enemies, controller.owner.username) &&
        range > LOCAL_SPHERE &&
        controller.owner.username !== "Invader"
      ) {
        delete Memory.targetRooms[room.name];
        log.a(
          "Abandoning attack on room " +
            roomLink(room.name) +
            " as they do not meet the required " +
            "threat level for a siege",
          "OPERATION PLANNER: "
        );
        return memory.recycle;
      }
  ...
  if (
    !cache[room.name] ||
    !cache[room.name].type ||
    cache[room.name].type === "attack" ||
    cache[room.name].type === "scout"
  ) {
    delete Memory.targetRooms[room.name];
  } else {
    log.a(
      cache[room.name].type + " planned for room " + roomLink(room.name),
      "OPERATION PLANNER: "
    );
...
function nukeTarget(room) {
  let nukes = _.filter(
    ...
    (s) =>
      ...
      !s.cooldown &&
      Game.map.getRoomLinearDistance(s.room.name, room.name) <= 10
  );
  if (nukes.length) {
    let launched = 0;
    let nukesNeeded;
    for (let nuke of nukes) {
      ...
      let clusteredTower = _.filter(
        room.structures,
        (s) =>
          s.structureType === STRUCTURE_TOWER &&
          s.pos.findInRange(room.structures, 4, {
            filter: (l) => l.structureType === STRUCTURE_TOWER,
          }).length >= 3 &&
          (!s.pos.checkForRampart() ||
            s.pos.checkForRampart().hits < nukes.length * 7500000)
      )[0];
      let clusteredSpawns = _.filter(
        room.structures,
        (s) =>
          s.structureType === STRUCTURE_SPAWN &&
          s.pos.findInRange(room.structures, 4, {
            filter: (l) => l.structureType === STRUCTURE_SPAWN,
          }).length >= 2 &&
          (!s.pos.checkForRampart() ||
            s.pos.checkForRampart().hits < nukes.length * 7500000)
      )[0];
      let terminal = _.filter(
        room.structures,
        (s) =>
      ...
      if (clusteredTower) {
        if (clusteredTower.pos.checkForRampart())
          nukesNeeded = _.ceil(
            clusteredTower.pos.checkForRampart().hits / 10000000
          );
        launched += 1;
        nuke.launchNuke(clusteredTower.pos);
        log.a(
          "NUCLEAR LAUNCH DETECTED - " +
            clusteredTower.pos.roomName +
            " " +
            clusteredTower.pos.x +
            "." +
            clusteredTower.pos.y +
            " has a nuke inbound from " +
            nuke.room.name +
            " and will impact in 50,000 ticks."
        );
        Game.notify(
          "NUCLEAR LAUNCH DETECTED - " +
            clusteredTower.pos.roomName +
        ...
        if (clusteredSpawns.pos.checkForRampart())
          nukesNeeded = _.ceil(
            clusteredSpawns.pos.checkForRampart().hits / 10000000
          );
        launched += 1;
        nuke.launchNuke(clusteredSpawns.pos);
        log.a(
          "NUCLEAR LAUNCH DETECTED - " +
            clusteredSpawns.pos.roomName +
            " " +
            clusteredSpawns.pos.x +
            "." +
            clusteredSpawns.pos.y +
            " has a nuke inbound from " +
            nuke.room.name +
            " and will impact in 50,000 ticks."
        );
        ...
        if (terminal.pos.checkForRampart())
          nukesNeeded = _.ceil(terminal.pos.checkForRampart().hits / 10000000);
        launched += 1;
