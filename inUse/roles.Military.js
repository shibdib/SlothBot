...
function attacker(creep) {
    ...
    if (creep.memory.squadLeader === true) {
        ...
        let weakPoint = creep.pos.findInRange(FIND_STRUCTURES, 10, {filter: (s) => (s.structureType === STRUCTURE_RAMPART || s.structureType === STRUCTURE_WALL) && _.includes(doNotAggress, s.room.controller.owner['username']) === false});
        let hostileStructures = creep.pos.findClosestByPath(FIND_STRUCTURES, {filter: (s) => (s.structureType !== STRUCTURE_RAMPART || s.structureType !== STRUCTURE_WALL) && _.includes(doNotAggress, s.room.controller.owner['username']) === false});
